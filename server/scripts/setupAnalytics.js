...
class AnalyticsSetup {
  ...
  async setupSQLiteTables() {
    ...
    const schemaPath = path.join(
      __dirname,
      '../database/migrations/analytics_schema_sqlite.sql'
    );
    const schema = await fs.readFile(schemaPath, 'utf8');
  ...
  async setupDefaultConfigurations() {
    ...
    try {
        ...
        try {
          ...
          if (existing.length === 0) {
            ...
            const config = {
              ...
              revenue_alert_threshold: -10.0,
              efficiency_alert_threshold: 80.0,
              churn_risk_alert_threshold: 0.7,
              allowed_exports: this.useSupabase
                ? ['csv', 'pdf', 'excel']
                : 'csv,pdf,excel',
            };
            ...
            this.log(
              `Analytics configuration already exists for shop: ${shop.name}`
            );
          }
        } catch (error) {
          this.log(
            `Warning: Could not create configuration for shop ${shop.name}: ${error.message}`,
            'warn'
          );
        }
      }
      ...
      this.log(
        `Warning: Could not setup default configurations: ${error.message}`,
        'warn'
      );
    }
  }
  ...
  async populateInitialData() {
    ...
    try {
      ...
      this.log(
        `Warning: Could not populate initial data: ${error.message}`,
        'warn'
      );
    }
  }
  ...
  async populateShopAnalytics(shopId) {
    try {
      ...
      this.log(
        `Warning: Could not populate analytics for shop ${shopId}: ${error.message}`,
        'warn'
      );
    }
  }
    ...
    try {
      ...
      const jobsInProgress = await databaseService.query('jobs', {
        where: {
          shop_id: shopId,
          status: {
            in: ['body_structure', 'paint_prep', 'paint_booth', 'reassembly'],
          },
        },
      });
      ...
      const revenueLabor = jobsCompleted.reduce(
        (sum, job) => sum + (parseFloat(job.labor_amount) || 0),
        0
      );
      const revenueParts = jobsCompleted.reduce(
        (sum, job) => sum + (parseFloat(job.parts_amount) || 0),
        0
      );
      const avgCycleTime =
        jobsCompleted.length > 0
          ? jobsCompleted.reduce((sum, job) => sum + (job.cycle_time || 0), 0) /
      ...
      this.log(
        `Could not generate daily metrics for ${targetDate.toDateString()}: ${error.message}`,
        'warn'
      );
    }
  }
  ...
  async generateCustomerAnalytics(shopId) {
    try {
      ...
      for (const customer of customers) {
        ...
        else if (totalSpent > 5000 && daysSinceLastJob < 180)
          segment = 'High Value';
        ...
        const analyticsData = {
          ...
          first_job_date:
            jobs.length > 0
              ? new Date(Math.min(...jobs.map(j => new Date(j.created_at))))
              : null,
        ...
        if (existing.length > 0) {
          await databaseService.update('customer_analytics', analyticsData, {
            id: existing[0].id,
          });
        } else {
      ...
      this.log(
        `Could not generate customer analytics: ${error.message}`,
        'warn'
      );
    }
  }
  ...
  async createAnalyticsViews() {
    ...
    try {
      ...
      this.log(
        `Warning: Could not refresh analytics views: ${error.message}`,
        'warn'
      );
    }
  }
