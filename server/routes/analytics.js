...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const currentData = await analyticsService.getRevenueAnalytics(
        shopId,
        currentPeriod,
        groupBy
      );
      ...
      const comparison = {
        ...
        change: {
          revenue: calculatePercentageChange(
            currentData.totals.revenue,
            comparisonData.totals.revenue
          ),
          jobs: calculatePercentageChange(
            currentData.totals.jobs,
            comparisonData.totals.jobs
          ),
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const analytics = await analyticsService.getTechnicianAnalytics(
        shopId,
        period
      );
      ...
      if (!canViewAll) {
        filteredAnalytics = analytics.filter(
          tech => tech.technician_id === req.user.id
        );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const leaderboard = sortedAnalytics
...
router.get(
  ...
  async (req, res) => {
    try {
      const { shopId } = req.user;
      const { category, abcClass, velocityClass, lowStock = false } = req.query;
      ...
      if (abcClass) {
        filteredAnalytics = filteredAnalytics.filter(
          part => part.abc_class === abcClass
        );
      }

      if (velocityClass) {
        filteredAnalytics = filteredAnalytics.filter(
          part => part.velocity_class === velocityClass
        );
      }
  ...
  async (req, res) => {
    try {
      ...
      const executiveSummary = {
        ...
        alerts: [
          ...(dashboardStats.jobs?.overdue > 0
            ? [`${dashboardStats.jobs.overdue} jobs are overdue`]
            : []),
          ...(partsAnalytics.lowStockParts.length > 0
            ? [`${partsAnalytics.lowStockParts.length} parts are low in stock`]
            : []),
          ...(customerAnalytics.summary.high_risk_customers > 0
            ? [
                `${customerAnalytics.summary.high_risk_customers} customers at risk of churning`,
              ]
            : []),
        ],
      };
      ...
      for (const metric of metrics) {
        switch (metric) {
          case 'revenue':
            ...
            break;
          case 'customers':
            customReport.data.customers =
              await analyticsService.getCustomerAnalytics(shopId);
            break;
          case 'technicians':
            customReport.data.technicians =
            ...
            break;
          case 'parts':
            customReport.data.parts =
              await analyticsService.getPartsAnalytics(shopId);
            break;
          default:
...
async function generateRevenueForecast(timeline) {
  ...
  const avgGrowth =
    lastThreeMonths.reduce((sum, month) => sum + (month.growth_rate || 0), 0) /
    3;
...
function generateInventoryRecommendations(analytics) {
  ...
  const slowMovingParts = analytics.fullAnalytics.filter(
    part =>
      part.velocity_class === 'Slow' &&
      part.current_stock > part.optimal_stock_level
  );
