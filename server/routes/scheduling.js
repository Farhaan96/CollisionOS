...
router.get('/capacity', async (req, res) => {
  try {
    ...
    for (const dept of departments) {
      ...
      const dept_config = config.department_capacity?.[dept] || {
        daily_hours: 8,
        weekly_hours: 40,
      };
      const dept_workload = workload.filter(w => w.department === dept);
      const dept_technicians = technicians.filter(t =>
        t.departments.includes(dept)
      );

      // Calculate capacity metrics
      const total_available_hours = dept_technicians.reduce(
        (sum, tech) => sum + tech.available_hours,
        0
      );
      const scheduled_hours = dept_workload.reduce(
        (sum, work) => sum + work.estimated_hours,
        0
      );
      const utilization =
        total_available_hours > 0
          ? (scheduled_hours / total_available_hours) * 100
          : 0;
      ...
      if (dept_technicians.filter(t => t.available_hours > 0).length === 0)
        bottlenecks.push('all_technicians_busy');

      // Calculate next available slot
      const next_available = calculateNextAvailableSlot(
        dept_technicians,
        scheduled_hours,
        dept_config
      );
      ...
      capacity_by_department[dept] = {
        ...
        available_technicians: dept_technicians.filter(
          t => t.available_hours > 0
        ).length,
        ...
        average_job_duration:
          dept_workload.length > 0
            ? (scheduled_hours / dept_workload.length).toFixed(1)
            : 0,
    ...
    const overall_metrics = calculateOverallCapacityMetrics(
...
router.post('/book', schedulingRateLimit, async (req, res) => {
  try {
    ...
    const parts_constraints = await checkPartsConstraints(
      ro_id,
      parts_availability
    );
...
router.get('/technicians', async (req, res) => {
  try {
    ...
    const available_technicians = technician_details
      ...
      .sort(
        (a, b) =>
          b.availability.available_hours - a.availability.available_hours
      );

    // Calculate department summary
    const department_summary = calculateDepartmentSkillsSummary(
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          fully_available: available_technicians.filter(
            t => t.availability.available_hours >= 6
          ).length,
          partially_available: available_technicians.filter(
            t =>
              t.availability.available_hours > 0 &&
              t.availability.available_hours < 6
          ).length,
          fully_booked: available_technicians.filter(
            t => t.availability.available_hours === 0
          ).length,
...
router.post('/what-if', schedulingRateLimit, async (req, res) => {
  try {
    ...
    for (const scenario of scenarios) {
      const analysis = await analyzeSchedulingScenario(
        shopId,
        scenario,
        comparison_mode
      );
    ...
    const comparison_report = generateScenarioComparison(
      scenario_results,
      comparison_mode
    );
  ...
  try {
    ...
    const confidence_analysis =
      include_confidence === 'true'
        ? await analyzeETAConfidence(repair_order, eta_calculation)
        : null;

    // Get detailed breakdown
    const eta_breakdown =
      breakdown === 'true'
        ? await getETABreakdown(repair_order, eta_calculation)
        : null;
...
async function getTechnicianSchedule(technicianId, date) {
  ...
  const scheduled_hours = scheduled_work.reduce(
    (sum, work) => sum + work.estimated_hours,
    0
  );
...
function calculateOverallCapacityMetrics(department_capacities) {
  ...
  return {
    total_technicians: departments.reduce(
      (sum, dept) => sum + dept.total_technicians,
      0
    ),
    overall_utilization:
      departments.length > 0
        ? (
            departments.reduce(
              (sum, dept) => sum + parseFloat(dept.utilization_percentage),
              0
            ) / departments.length
          ).toFixed(1)
        : '0.0',
    bottleneck_departments: departments
      .filter(dept => dept.bottlenecks.length > 0)
      .map(dept => dept.department),
    total_jobs_queued: departments.reduce(
      (sum, dept) => sum + dept.jobs_in_queue,
      0
    ),
  };
}
...
async function findOptimalSchedule(
  shopId,
  operations,
  priority,
  customer_requested_date,
  ...
  for (const operation of operations) {
    ...
    if (available_techs.length === 0) {
      feasible = false;
      conflicts.push(
        `No technicians available for ${operation.operation_type}`
      );
      continue;
    }
    ...
    const scheduled_end = new Date(
      current_time.getTime() + operation.estimated_hours * 60 * 60 * 1000
    );
  ...
  return {
    ...
    estimated_completion:
      schedule.length > 0 ? schedule[schedule.length - 1].scheduled_end : null,
...
function calculateDepartmentSkillsSummary(technicians, department_filter) {
  ...
  for (const dept of departments) {
    ...
    const dept_techs = technicians.filter(tech =>
      tech.departments.includes(dept)
    );
    summary[dept] = {
      technician_count: dept_techs.length,
      total_available_hours: dept_techs.reduce(
        (sum, tech) => sum + tech.availability.available_hours,
        0
      ),
      average_skill_level: dept_techs.length > 0 ? 'intermediate' : 'none',
      certifications_coverage: [
        ...new Set(dept_techs.flatMap(tech => tech.certifications)),
      ],
    };
  }
...
async function calculateSmartETA(repair_order, shopId) {
  ...
  const base_hours =
    repair_order.productionWorkflow?.reduce(
      (sum, work) => sum + work.estimated_hours,
      0
    ) || 40;
  const completion_date = new Date();
  completion_date.setDate(
    completion_date.getDate() + Math.ceil(base_hours / 8)
  );
