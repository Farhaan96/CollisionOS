...
module.exports = (sequelize) => {
  return sequelize.define('LoanerReservation', {
    id: {
    ...
    reservationStatus: {
      type: DataTypes.ENUM(
        'pending', 'confirmed', 'active', 'completed', 'cancelled', 'no_show', 'early_return'
      ),
    ...
    eligibilitySource: {
      type: DataTypes.ENUM('insurance', 'warranty', 'goodwill', 'customer_pay'),
      allowNull: true
    },
    authorizationNumber: {
      type: DataTypes.STRING(100),
    ...
    coverageType: {
      type: DataTypes.ENUM('full_coverage', 'basic_coverage', 'liability_only', 'customer_insurance', 'none'),
      allowNull: true
    },
    dailyAllowance: {
    ...
    paymentMethod: {
      type: DataTypes.ENUM('cash', 'credit_card', 'debit_card', 'check', 'insurance_direct', 'account_billing'),
      allowNull: true
    },
    creditCardOnFile: {
      type: DataTypes.STRING(100),
    ...
    indexes: [
      ...
      { fields: ['loanerVehicleId'] },
      { fields: ['claimManagementId'] },
      { fields: ['reservationNumber'], unique: true },
      { fields: ['reservationStatus'] },
      { fields: ['requestedStartDate'] },
      { fields: ['requestedEndDate'] },
      { fields: ['confirmedStartDate'] },
      { fields: ['confirmedEndDate'] },
      { fields: ['actualPickupDate'] },
      { fields: ['actualReturnDate'] },
      { fields: ['eligibilitySource'] },
      { fields: ['primaryDriverName'] },
      { fields: ['primaryDriverLicense'] },
      { fields: ['hasConflict'] },
      { fields: ['isEmergencyReservation'] },
      { fields: ['createdAt'] },
      { fields: ['customerId', 'createdAt'] },
      { fields: ['loanerVehicleId', 'requestedStartDate'] },
      { fields: ['reservationStatus', 'requestedStartDate'] }
    ]
  });
};