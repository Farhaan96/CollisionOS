...
module.exports = (sequelize, Sequelize) => {
  const WorkflowStatus = sequelize.define('WorkflowStatus', {
    ...
    // Status and timing
    status: {
      type: DataTypes.ENUM('pending', 'in_progress', 'completed', 'skipped', 'on_hold', 'rework', 'failed'),
      allowNull: false,
    ...
    hooks: {
      ...
      beforeUpdate: (workflow) => {
        ...
        if (workflow.changed('status')) {
          ...
          if (workflow.status === 'completed' && !workflow.completedAt) {
            ...
            if (workflow.startedAt) {
              const duration = Math.round((now - new Date(workflow.startedAt)) / (1000 * 60));
        ...
        if (workflow.changed('inspectionCompleted') && workflow.inspectionCompleted) {
        ...
        if (workflow.changed('customerNotified') && workflow.customerNotified) {
