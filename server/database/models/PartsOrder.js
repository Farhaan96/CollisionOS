...
module.exports = (sequelize, Sequelize) => {
  const PartsOrder = sequelize.define('PartsOrder', {
    ...
    // Status tracking
    status: {
      type: DataTypes.ENUM('draft', 'pending', 'sent', 'confirmed', 'in_production', 'shipped', 'partially_received', 'received', 'backordered', 'cancelled', 'returned'),
      allowNull: false,
      defaultValue: 'draft'
    },
    priority: {
    ...
    shippingMethod: {
      type: DataTypes.ENUM('pickup', 'standard', 'expedited', 'overnight', 'freight', 'delivery', 'other'),
    ...
    // Payment information
    paymentTerms: {
      type: DataTypes.ENUM('cod', 'net_15', 'net_30', 'net_60', 'prepaid', 'credit_card', 'account'),
      defaultValue: 'account'
    },
    paymentStatus: {
      type: DataTypes.ENUM('unpaid', 'partial', 'paid', 'overdue', 'disputed'),
      defaultValue: 'unpaid'
    },
    paymentDate: {
      type: DataTypes.DATE,
    ...
    paymentMethod: {
      type: DataTypes.ENUM('cash', 'check', 'credit_card', 'ach', 'wire', 'account_credit', 'other'),
      allowNull: true
    },
    ...
    hooks: {
      ...
      beforeUpdate: (order) => {
        ...
        if (order.changed('status')) {
          ...
          if (['received', 'partially_received'].includes(order.status) && !order.receivedDate) {
        ...
        const financialFields = ['subtotal', 'taxAmount', 'shippingAmount', 'handlingAmount', 'discountAmount'];
  ...
  PartsOrder.prototype.isOverdue = function() {
    ...
    return new Date() > new Date(this.expectedDate) && !['received', 'cancelled'].includes(this.status);
  };
