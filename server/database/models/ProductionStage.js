...
module.exports = (sequelize, Sequelize) => {
  const ProductionStage = sequelize.define('ProductionStage', {
    ...
    stageType: {
      type: DataTypes.ENUM('production', 'quality', 'administrative', 'customer_interaction', 'external'),
      allowNull: false,
      defaultValue: 'production'
    },
    category: {
      type: DataTypes.ENUM('intake', 'disassembly', 'repair', 'paint', 'reassembly', 'quality', 'delivery'),
      allowNull: false
    },
    ...
    hooks: {
      beforeCreate: (stage) => {
        ...
        if (!stage.stageCode) {
          stage.stageCode = stage.stageName.toUpperCase().replace(/\s+/g, '_').substring(0, 20);
        }
      ...
      beforeUpdate: (stage) => {
        ...
        if (stage.changed('prerequisites') || stage.changed('dependentStages')) {
  ...
  ProductionStage.prototype.canStartForJob = function(jobWorkflowStatuses) {
    ...
    if (this.prerequisites && this.prerequisites.length > 0) {
      return this.prerequisites.every(prereqCode => {
        const prereqStatus = jobWorkflowStatuses.find(ws => ws.stage === prereqCode);
  ...
  ProductionStage.prototype.getEstimatedCompletionTime = function(startTime) {
    ...
    const completion = new Date(start.getTime() + (this.estimatedDuration + this.bufferTime) * 60000);
    return completion;
  };
  ...
  ProductionStage.prototype.validateEnvironmentalConditions = function(conditions) {
    ...
    if (requirements.temperature && conditions.temperature) {
      if (conditions.temperature < requirements.minTemperature || 
