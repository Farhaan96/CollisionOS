...
module.exports = (sequelize, Sequelize) => {
  const InsuranceCompany = sequelize.define('InsuranceCompany', {
    ...
    type: {
      type: DataTypes.ENUM('auto', 'commercial', 'specialty', 'self_insured', 'other'),
      defaultValue: 'auto'
    },
    // Contact information
    address: {
    ...
    paymentTerms: {
      type: DataTypes.ENUM('immediate', 'net_15', 'net_30', 'net_45', 'net_60'),
      defaultValue: 'net_30'
    },
    ...
    relationshipStatus: {
      type: DataTypes.ENUM('excellent', 'good', 'fair', 'poor', 'problematic'),
      defaultValue: 'good'
    },
    // Status and activity
    isActive: {
    ...
    hooks: {
      ...
      beforeUpdate: (company) => {
        ...
        if (company.changed('totalClaims') || company.changed('totalClaimsValue')) {
          if (company.totalClaims > 0) {
            company.avgClaimValue = (company.totalClaimsValue / company.totalClaims).toFixed(2);
          } else {
        ...
        if (company.changed('customerServiceRating') || company.changed('paymentRating')) {
          const ratings = [company.customerServiceRating, company.paymentRating].filter(r => r !== null);
          if (ratings.length > 0) {
            company.overallRating = (ratings.reduce((sum, rating) => sum + rating, 0) / ratings.length).toFixed(1);
          }
        }
      }
    }
  ...
  InsuranceCompany.prototype.needsAttention = function() {
    return this.relationshipStatus === 'problematic' || 
