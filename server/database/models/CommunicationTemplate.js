...
module.exports = (sequelize, Sequelize) => {
    ...
    },
    {
      ...
      hooks: {
        ...
        beforeUpdate: template => {
          ...
          if (
            template.changed('sentCount') ||
            template.changed('deliveredCount') ||
            template.changed('openedCount') ||
            template.changed('clickedCount')
          ) {
            if (template.sentCount > 0) {
              template.deliveryRate = (
                (template.deliveredCount / template.sentCount) *
                100
              ).toFixed(2);
            }
            ...
            if (template.deliveredCount > 0) {
              template.openRate = (
                (template.openedCount / template.deliveredCount) *
                100
              ).toFixed(2);
              template.clickRate = (
                (template.clickedCount / template.deliveredCount) *
                100
              ).toFixed(2);
              template.responseRate = (
                (template.responseCount / template.deliveredCount) *
                100
              ).toFixed(2);
            }
          }
  ...
  CommunicationTemplate.prototype.canSend = function () {
    return (
      this.status === 'active' && (!this.requiresApproval || this.approvedBy)
    );
  };
  ...
  CommunicationTemplate.prototype.renderContent = function (
    content,
    variables
  ) {
