...
module.exports = (sequelize, Sequelize) => {
  const FinancialTransaction = sequelize.define('FinancialTransaction', {
    ...
    processingStatus: {
      type: DataTypes.ENUM('queued', 'processing', 'processed', 'failed', 'retry'),
      defaultValue: 'queued'
    },
    // Dates and timing
    transactionDate: {
    ...
    hooks: {
      beforeCreate: (transaction) => {
        ...
        if (!transaction.transactionNumber) {
          transaction.transactionNumber = generateTransactionNumber(transaction.transactionType);
        }
        ...
        if (!transaction.effectiveDate) {
          transaction.effectiveDate = transaction.transactionDate || new Date();
        }
        
        // Calculate base currency amount if different
        if (transaction.currency !== 'USD' && transaction.exchangeRate !== 1.0) {
          transaction.baseAmount = (parseFloat(transaction.amount) * parseFloat(transaction.exchangeRate)).toFixed(2);
        }
      },
      beforeUpdate: (transaction) => {
        ...
        if (transaction.changed('approved') && transaction.approved && !transaction.approvedAt) {
          transaction.approvedAt = new Date();
        }
        
        // Recalculate net amount if fees changed
        const feeFields = ['processingFee', 'merchantFee', 'bankFee', 'otherFees'];
        if (feeFields.some(field => transaction.changed(field))) {
          transaction.netAmount = calculateNetAmount(transaction);
        }
        
        // Update dispute status
        if (transaction.changed('isDisputed') && transaction.isDisputed && !transaction.disputeDate) {
          transaction.disputeDate = new Date();
        }
        
        // Update reconciliation
        if (transaction.changed('isReconciled') && transaction.isReconciled && !transaction.reconciledDate) {
  ...
  FinancialTransaction.prototype.isIncoming = function() {
    return ['payment_received', 'refund_received', 'settlement'].includes(this.transactionType);
  };

  FinancialTransaction.prototype.isOutgoing = function() {
    return ['payment_sent', 'refund_issued', 'fee', 'penalty'].includes(this.transactionType);
  };
  ...
  FinancialTransaction.prototype.getFeePercentage = function() {
    ...
    return ((parseFloat(this.totalFees || 0) / parseFloat(this.amount)) * 100).toFixed(2);
  };

  FinancialTransaction.prototype.addToHistory = function(field, oldValue, newValue, userId = null) {
