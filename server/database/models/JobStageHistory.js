...
module.exports = (sequelize, Sequelize) => {
  const JobStageHistory = sequelize.define('JobStageHistory', {
    id: {
      type: DataTypes.UUID,
    ...
    movementType: {
      type: DataTypes.ENUM('forward', 'backward', 'skip', 'restart', 'parallel'),
      allowNull: false,
    ...
    hooks: {
      ...
      beforeUpdate: (history) => {
        ...
        if (history.changed('inspectionCompleted') && history.inspectionCompleted) {
        ...
        if (history.changed('stageEndTime') && history.stageStartTime && history.stageEndTime) {
          const start = new Date(history.stageStartTime);
          const end = new Date(history.stageEndTime);
          history.stageDuration = Math.round((end - start) / (1000 * 60));
        }
  ...
  JobStageHistory.prototype.getTotalDelayTime = function() {
    return (this.waitTime || 0) + (this.delayMinutes || 0) + (this.resolutionTime || 0);
  };
