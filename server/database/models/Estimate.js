...
module.exports = (sequelize, Sequelize) => {
  const Estimate = sequelize.define('Estimate', {
    ...
    // Status and workflow
    status: {
      type: DataTypes.ENUM('draft', 'pending_review', 'under_review', 'approved', 'rejected', 'revised', 'supplements_required', 'converted_to_job'),
      allowNull: false,
      defaultValue: 'draft'
    },
    estimateType: {
      type: DataTypes.ENUM('collision', 'hail', 'vandalism', 'theft', 'flood', 'mechanical', 'glass', 'paint', 'other'),
      allowNull: false,
    ...
    // Damage information
    damageType: {
      type: DataTypes.ENUM('minor', 'moderate', 'major', 'severe', 'total_loss'),
      allowNull: true
    },
    damageDescription: {
      type: DataTypes.TEXT,
    ...
    hooks: {
      ...
      beforeUpdate: (estimate) => {
        ...
        if (estimate.changed('customerApproval') && estimate.customerApproval) {
          estimate.customerApprovalDate = new Date();
        }
        
        // Update insurance approval date
        if (estimate.changed('insuranceApproval') && estimate.insuranceApproval) {
          estimate.insuranceApprovalDate = new Date();
        }
        
        // Recalculate totals if financial fields changed
        const financialFields = ['partsTotal', 'laborTotal', 'paintTotal', 'subletTotal', 'materialTotal', 'otherTotal', 'taxTotal'];
  ...
  Estimate.prototype.getProfitMargin = function() {
    const totalCost = parseFloat(this.partsTotal) + parseFloat(this.materialTotal);
