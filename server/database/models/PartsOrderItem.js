...
module.exports = (sequelize, Sequelize) => {
  const PartsOrderItem = sequelize.define('PartsOrderItem', {
    id: {
      type: DataTypes.UUID,
    ...
    partType: {
      type: DataTypes.ENUM('OEM', 'Aftermarket', 'Used', 'Reconditioned', 'Generic'),
      allowNull: false,
      defaultValue: 'OEM'
    },
    category: {
      type: DataTypes.ENUM('body', 'frame', 'mechanical', 'electrical', 'glass', 'interior', 'exterior', 'paint', 'hardware', 'other'),
      allowNull: true
    },
    // Quantities
    quantityOrdered: {
    ...
    // Status tracking
    status: {
      type: DataTypes.ENUM('pending', 'ordered', 'confirmed', 'shipped', 'received', 'backordered', 'cancelled', 'returned', 'damaged'),
      allowNull: false,
    ...
    condition: {
      type: DataTypes.ENUM('new', 'used', 'reconditioned', 'damaged', 'defective'),
      defaultValue: 'new'
    },
    qualityGrade: {
    ...
    returnReason: {
      type: DataTypes.ENUM('wrong_part', 'damaged', 'defective', 'no_longer_needed', 'customer_change', 'warranty', 'other'),
      allowNull: true
    },
    returnNotes: {
      type: DataTypes.TEXT,
    ...
    hooks: {
      ...
      beforeUpdate: (item) => {
        ...
        if (item.changed('status') && item.status === 'received' && !item.receivedDate) {
          item.receivedDate = new Date();
        }
        
        // Update shipped date when status changes to shipped
        if (item.changed('status') && item.status === 'shipped' && !item.shippedDate) {
          item.shippedDate = new Date();
        }
        
        // Set installation date when marked as installed
        if (item.changed('installed') && item.installed && !item.installedDate) {
          item.installedDate = new Date();
        }
        
        // Set return deadline when received
        if (item.changed('receivedDate') && item.receivedDate && item.returnable && !item.returnDeadline) {
          const deadline = new Date(item.receivedDate);
          deadline.setDate(deadline.getDate() + 30);
          item.returnDeadline = deadline;
        }
        ...
        if (item.changed('quantityOrdered') || item.changed('unitCost') || item.changed('discount') || item.changed('discountAmount')) {
          item.totalCost = calculateItemTotal(item);
        }
      }
    }
  ...
  PartsOrderItem.prototype.isOverdue = function() {
    ...
    return new Date() > new Date(this.expectedDate) && !['received', 'cancelled', 'returned'].includes(this.status);
  };

  PartsOrderItem.prototype.canBeReturned = function() {
    if (!this.returnable) return false;
    if (this.returnDeadline && new Date() > new Date(this.returnDeadline)) return false;
  ...
  PartsOrderItem.prototype.getWarrantyExpiry = function() {
    if (!this.installedDate || (!this.warrantyMonths && !this.warrantyMiles)) return null;
...
function calculateItemTotal(item) {
  let total = parseFloat(item.unitCost || 0) * parseFloat(item.quantityOrdered || 0);
