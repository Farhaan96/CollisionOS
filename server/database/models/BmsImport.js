...
module.exports = (sequelize, Sequelize) => {
  const BmsImport = sequelize.define('BmsImport', {
    id: {
      type: DataTypes.UUID,
    ...
    status: {
      type: DataTypes.ENUM('pending', 'processing', 'success', 'failed', 'partial', 'cancelled'),
      allowNull: false,
    ...
    hooks: {
      beforeCreate: (bmsImport) => {
        if (!bmsImport.processingStarted && bmsImport.status === 'processing') {
          bmsImport.processingStarted = new Date();
        }
      },
      beforeUpdate: (bmsImport) => {
        if (bmsImport.changed('status')) {
          if (bmsImport.status === 'processing' && !bmsImport.processingStarted) {
            bmsImport.processingStarted = new Date();
          }
          
          if (['success', 'failed', 'partial'].includes(bmsImport.status) && bmsImport.processingStarted && !bmsImport.processingCompleted) {
            bmsImport.processingCompleted = new Date();
            const duration = Math.floor((bmsImport.processingCompleted - new Date(bmsImport.processingStarted)) / 1000);
  ...
  BmsImport.prototype.hasErrors = function() {
    return this.errorRecords > 0 || (this.validationErrors && this.validationErrors.length > 0);
  };

  BmsImport.prototype.canBeRolledBack = function() {
    return this.canRollback && !this.rolledBack && ['success', 'partial'].includes(this.status);
  };
