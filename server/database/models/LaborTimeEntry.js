...
module.exports = (sequelize, Sequelize) => {
  const LaborTimeEntry = sequelize.define(
    'LaborTimeEntry',
    {
      id: {
        type: DataTypes.UUID,
        ...
        beforeUpdate: entry => {
          ...
          if (entry.changed('clockOut') && entry.clockOut) {
            ...
            if (entry.estimatedHours && entry.hoursWorked) {
              entry.efficiency = (
                (entry.estimatedHours / entry.hoursWorked) *
                100
              ).toFixed(2);
              entry.varianceHours = (
                entry.hoursWorked - entry.estimatedHours
              ).toFixed(2);
            }

            // Calculate labor cost
            if (entry.hourlyRate && entry.hoursWorked) {
              entry.laborCost = (entry.hourlyRate * entry.hoursWorked).toFixed(
                2
              );
            }
            ...
            if (entry.customerRate && entry.billableHours) {
              entry.billableAmount = (
                entry.customerRate * entry.billableHours
              ).toFixed(2);
            }
          }
          ...
          if (entry.changed('breakStart') || entry.changed('breakEnd')) {
            if (entry.breakStart && entry.breakEnd) {
              const breakDuration =
                (new Date(entry.breakEnd) - new Date(entry.breakStart)) /
                (1000 * 60 * 60);
