...
module.exports = (sequelize, Sequelize) => {
  const Invoice = sequelize.define('Invoice', {
    ...
    paymentStatus: {
      type: DataTypes.ENUM('unpaid', 'partial', 'paid', 'overdue', 'cancelled', 'refunded'),
      allowNull: false,
      defaultValue: 'unpaid'
    },
    // Payment breakdown
    ...
    status: {
      type: DataTypes.ENUM('draft', 'sent', 'viewed', 'paid', 'overdue', 'cancelled', 'voided'),
      allowNull: false,
      defaultValue: 'draft'
    },
    invoiceType: {
      type: DataTypes.ENUM('standard', 'insurance', 'warranty', 'supplement', 'final'),
      allowNull: false,
    ...
    paymentTerms: {
      type: DataTypes.ENUM('due_on_receipt', 'net_15', 'net_30', 'net_60', 'cod'),
    ...
    hooks: {
      beforeCreate: (invoice) => {
        ...
        invoice.amountDue = parseFloat(invoice.totalAmount) - parseFloat(invoice.amountPaid);
      },
      beforeUpdate: (invoice) => {
        ...
        if (invoice.dueDate && new Date() > new Date(invoice.dueDate) && due > 0) {
          invoice.paymentStatus = 'overdue';
          const today = new Date();
          const dueDate = new Date(invoice.dueDate);
          invoice.daysOverdue = Math.ceil((today - dueDate) / (1000 * 60 * 60 * 24));
        }
        ...
        const financialFields = ['laborAmount', 'partsAmount', 'paintAmount', 'materialAmount', 'subletAmount', 'miscAmount', 'discountAmount', 'taxAmount'];
  ...
  Invoice.prototype.isOverdue = function() {
    ...
    return new Date() > new Date(this.dueDate) && parseFloat(this.amountDue) > 0;
  };
  ...
  Invoice.prototype.canBeVoided = function() {
    return ['draft', 'sent', 'viewed'].includes(this.status) && parseFloat(this.amountPaid) === 0;
  };
  ...
  Invoice.prototype.requiresCustomerPayment = function() {
    return parseFloat(this.customerPayment) > 0 || parseFloat(this.deductible) > 0;
  };
...
function calculateInvoiceTotal(invoice) {
  ...
  const total = subtotal - parseFloat(invoice.discountAmount || 0) + parseFloat(invoice.taxAmount || 0) + parseFloat(invoice.lateFeeAmount || 0);
