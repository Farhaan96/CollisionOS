...
module.exports = (sequelize, Sequelize) => {
  const Attachment = sequelize.define('Attachment', {
    ...
    // Status and workflow
    status: {
      type: DataTypes.ENUM('uploaded', 'processing', 'ready', 'archived', 'deleted'),
    ...
    // Storage information
    storageProvider: {
      type: DataTypes.ENUM('local', 'aws_s3', 'google_cloud', 'azure', 'other'),
      defaultValue: 'local'
    },
    storageKey: {
      type: DataTypes.STRING(500),
    ...
    hooks: {
      ...
      beforeUpdate: (attachment) => {
        ...
        if (attachment.changed('filePath') || attachment.changed('fileName')) {
