...
module.exports = (sequelize, Sequelize) => {
  const CommunicationLog = sequelize.define('CommunicationLog', {
    ...
    channel: {
      type: DataTypes.ENUM('sms', 'email', 'phone', 'push', 'portal', 'in_person', 'fax', 'mail', 'other'),
      allowNull: false
    },
    direction: {
    ...
    recipientType: {
      type: DataTypes.ENUM('customer', 'insurance', 'vendor', 'technician', 'manager', 'admin', 'other'),
      allowNull: false,
    ...
    hooks: {
      ...
      beforeUpdate: (log) => {
        ...
        if (log.changed('optOutReceived') && log.optOutReceived && !log.optOutTimestamp) {
  ...
  CommunicationLog.prototype.isSuccessful = function() {
    return ['sent', 'delivered', 'opened', 'clicked', 'replied'].includes(this.status);
  };
  ...
  CommunicationLog.prototype.getEngagementScore = function() {
    ...
    if (this.customerSatisfaction && this.customerSatisfaction >= 4) score += 20;
  ...
  CommunicationLog.prototype.markAsClicked = function(clickData = {}) {
    return this.update({
      ...
      clickedLinks: [...(this.clickedLinks || []), clickData.url].filter(Boolean)
    });
  };

  CommunicationLog.prototype.addResponse = function(responseContent, sentiment = null) {
    return this.update({
      status: 'replied',
