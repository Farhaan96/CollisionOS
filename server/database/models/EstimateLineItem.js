...
module.exports = (sequelize, Sequelize) => {
  const EstimateLineItem = sequelize.define('EstimateLineItem', {
    ...
    type: {
      type: DataTypes.ENUM('part', 'labor', 'paint', 'material', 'sublet', 'other', 'tax', 'discount'),
      allowNull: false
    },
    category: {
      type: DataTypes.ENUM('body', 'frame', 'mechanical', 'electrical', 'glass', 'interior', 'exterior', 'paint', 'refinish', 'other'),
      allowNull: true
    },
    operationCode: {
    ...
    laborType: {
      type: DataTypes.ENUM('body', 'paint', 'frame', 'mechanical', 'electrical', 'glass', 'detail', 'other'),
      allowNull: true
    },
    skillLevel: {
    ...
    paintType: {
      type: DataTypes.ENUM('basecoat', 'clearcoat', 'primer', 'sealer', 'adhesion_promoter', 'other'),
      allowNull: true
    },
    coverage: {
    ...
    // Status and workflow
    status: {
      type: DataTypes.ENUM('pending', 'approved', 'rejected', 'on_hold', 'supplement'),
    ...
    hooks: {
      beforeCreate: (lineItem) => {
        ...
        if (lineItem.type === 'labor' && !lineItem.laborRate) {
          lineItem.laborRate = 75.00; // Default labor rate
        }
      ...
      beforeUpdate: (lineItem) => {
        ...
        if (lineItem.changed('quantity') || lineItem.changed('unitPrice') || lineItem.changed('discount') || lineItem.changed('discountAmount')) {
          lineItem.totalPrice = calculateLineItemTotal(lineItem);
        }
  ...
  EstimateLineItem.prototype.isSupplement = function() {
    return !this.includedInOriginal || (this.supplementNumber && this.supplementNumber > 0);
  };
...
function calculateLineItemTotal(lineItem) {
  let total = parseFloat(lineItem.unitPrice || 0) * parseFloat(lineItem.quantity || 0);
