...
const createJobSchema = Joi.object({
  ...
  estimateAmount: Joi.number().precision(2).min(0).optional().messages({
    'number.base': 'Estimate amount must be a number',
    'number.min': 'Estimate amount cannot be negative',
  }),

  totalAmount: Joi.number().precision(2).min(0).optional().messages({
  ...
  repairNotes: Joi.string().max(2000).optional(),

  priority: Joi.string()
    .valid('low', 'medium', 'high', 'urgent')
    .default('medium'),

  estimatedCompletionDate: Joi.date().iso().optional(),

  actualCompletionDate: Joi.date().iso().optional(),

  insuranceClaimNumber: Joi.string().max(100).optional(),

  insuranceCompany: Joi.string().max(255).optional(),

  deductible: Joi.number().precision(2).min(0).optional(),

  laborHours: Joi.number().precision(2).min(0).optional(),

  laborRate: Joi.number().precision(2).min(0).optional(),
...
const updateJobSchema = Joi.object({
  ...
  estimateAmount: Joi.number().precision(2).min(0).optional(),
