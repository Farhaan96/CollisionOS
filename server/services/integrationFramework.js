  ...
  setupInterceptors() {
    ...
    this.client.interceptors.request.use(
      config => {
        ...
        // Log request
        console.log(
          `📤 ${this.name} API Request: ${config.method?.toUpperCase()} ${config.url}`
        );
    ...
    this.client.interceptors.response.use(
      response => {
        console.log(
          `✅ ${this.name} API Response: ${response.status} ${response.statusText}`
        );
      ...
      async error => {
        ...
        if (
          ...
          const retryAfter =
            error.response.headers['retry-after'] || this.rateLimitDelay / 1000;
          console.warn(
            `⏳ ${this.name} Rate limited, retrying after ${retryAfter}s`
          );
        ...
        if (this.shouldRetry(error) && !originalRequest._retry) {
          ...
          if (originalRequest._retryCount <= this.retryAttempts) {
            ...
            console.warn(
              `🔄 ${this.name} Retry ${originalRequest._retryCount}/${this.retryAttempts} after ${delay}ms`
            );
    ...
    switch (this.authType) {
      ...
      case 'custom':
        if (this.credentials.customHeader) {
          config.headers[this.credentials.customHeader.name] =
            this.credentials.customHeader.value;
        }
        break;
    }
    return config;
...
class IntegrationManager extends EventEmitter {
  ...
  registerProvider(name, client) {
    ...
    client.on('response', data =>
      this.emit('response', { provider: name, ...data })
    );
  ...
  async handleWebhook(provider, eventType, payload, signature = null) {
    try {
      ...
      if (signature) {
        ...
        if (
          !providerInstance.verifyWebhookSignature(
