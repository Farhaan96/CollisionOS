...
class AnalyticsService {
    ...
    async getSQLiteDashboardStats(shopId, dateFilter) {
        ...
        const jobStats = await databaseService.query('jobs', {
            select: 'id, status, total_amount, cycle_time, created_at, completion_date',
            where: { 
                shop_id: shopId,
        ...
        stats.jobs = {
            ...
            active: jobStats.filter(j => ['body_structure', 'paint_prep', 'paint_booth', 'reassembly'].includes(j.status)).length,
            completed: jobStats.filter(j => j.status === 'delivered').length,
            revenue: jobStats.filter(j => j.status === 'delivered').reduce((sum, j) => sum + (j.total_amount || 0), 0),
            avg_cycle_time: this.calculateAverage(jobStats.filter(j => j.cycle_time).map(j => j.cycle_time))
        };
        ...
        stats.customers = {
            ...
            new_period: customerStats.filter(c => new Date(c.created_at) >= dateFilter.start).length
        };
        ...
        stats.parts = {
            low_stock: partsStats.filter(p => p.current_stock <= p.minimum_stock).length,
            reorder_needed: partsStats.filter(p => p.current_stock <= p.reorder_point).length
        };
    ...
    async getRevenueAnalytics(shopId, period = 'year', groupBy = 'month') {
        try {
            ...
            if (this.useSupabase) {
                return await this.getSupabaseRevenueAnalytics(shopId, dateFilter, groupBy);
            } else {
                return await this.getSQLiteRevenueAnalytics(shopId, dateFilter, groupBy);
            }
        } catch (error) {
    ...
    async getSQLiteRevenueAnalytics(shopId, dateFilter, groupBy) {
        const jobs = await databaseService.query('jobs', {
            select: 'total_amount, labor_amount, parts_amount, materials_amount, created_at, completion_date',
            where: {
                shop_id: shopId,
                status: 'delivered',
                completion_date: { 
    ...
    processRevenueData(data, groupBy) {
        ...
        timeline.forEach((item, index) => {
            if (index > 0) {
                ...
                item.growth_rate = prev.revenue > 0 ? 
                    ((item.revenue - prev.revenue) / prev.revenue) * 100 : 0;
            } else {
                item.growth_rate = 0;
            }
        });
        ...
        return {
            ...
            averages: {
                ...
                revenue_per_period: timeline.length > 0 ? totals.revenue / timeline.length : 0
            }
        };
    }
    ...
    async getSQLiteCustomerAnalytics(shopId, options) {
        ...
        for (const customer of customers) {
            ...
            const totalSpent = completedJobs.reduce((sum, j) => sum + (j.total_amount || 0), 0);
            ...
            const churnRisk = this.calculateChurnRisk(daysSinceLastJob, avgJobValue, jobs.length);

            // Customer segmentation
            const segment = this.determineCustomerSegment(totalSpent, daysSinceLastJob, jobs.length);
            ...
            analytics.push({
                ...
                segment,
                lifetime_value: this.calculateLifetimeValue(totalSpent, jobs.length, daysSinceLastJob)
            });
        }
    ...
    processCustomerAnalytics(segments, analytics) {
        ...
        // Summary statistics
        const totalLTV = analytics.reduce((sum, c) => sum + (c.lifetime_value || 0), 0);
        const avgLTV = analytics.length > 0 ? totalLTV / analytics.length : 0;
        const avgChurnRisk = analytics.length > 0 ? 
            analytics.reduce((sum, c) => sum + (c.churn_risk_score || 0), 0) / analytics.length : 0;
    ...
    async getSQLiteTechnicianAnalytics(shopId, dateFilter) {
        ...
        for (const tech of technicians) {
            ...
            const avgEfficiency = completedJobs.length > 0 ?
                completedJobs.reduce((sum, j) => sum + (j.efficiency || 100), 0) / completedJobs.length : 100;
            ...
            analytics.push({
                ...
                completion_rate: jobs.length > 0 ? (completedJobs.length / jobs.length) * 100 : 0
            });
        }
    ...
    async getSQLitePartsAnalytics(shopId) {
        ...
        for (const part of parts) {
            ...
            const quantityUsed30d = jobParts.reduce((sum, jp) => sum + (jp.quantity_needed || 0), 0);
            const totalRevenue30d = jobParts.reduce((sum, jp) => sum + (jp.total_price || 0), 0);
            const totalCost30d = jobParts.reduce((sum, jp) => sum + (jp.total_cost || 0), 0);
            const margin30d = totalRevenue30d - totalCost30d;
            const marginPercentage = totalRevenue30d > 0 ? (margin30d / totalRevenue30d) * 100 : 0;

            // Calculate turnover rate
            const avgStock = part.current_stock > 0 ? part.current_stock : 1;
            const turnoverRate = quantityUsed30d > 0 ? (quantityUsed30d * 12) / avgStock : 0;
            ...
            analytics.push({
                ...
                abc_class: abcClass,
                velocity_class: turnoverRate > 6 ? 'Fast' : turnoverRate > 2 ? 'Medium' : 'Slow'
            });
        }
    ...
    processPartsAnalytics(data) {
        ...
        data.forEach(part => {
            ...
            summary.total_inventory_value += (part.current_stock || 0) * (part.cost_price || 0);
        });

        summary.avg_margin = data.length > 0 ?
            data.reduce((sum, p) => sum + (p.margin_percentage || 0), 0) / data.length : 0;
    ...
    calculateLifetimeValue(totalSpent, totalJobs, daysSinceLastJob) {
        ...
        const avgJobsPerYear = totalJobs > 1 ? Math.max(1, totalJobs * (365 / Math.max(365, daysSinceLastJob))) : 1;
    ...
    async calculateAndStoreDailyMetrics(shopId, targetDate) {
        ...
        const revenueTotal = jobsCompleted.reduce((sum, job) => sum + (job.total_amount || 0), 0);
        const avgCycleTime = this.calculateAverage(jobsCompleted.map(job => job.cycle_time).filter(Boolean));
        ...
        if (existingMetric.length > 0) {
            await databaseService.update('daily_metrics', metricsData, { id: existingMetric[0].id });
        } else {
