...
class CollisionOSAssistant {
  ...
  async processQuery(query, shopId, userId) {
    try {
      console.log(
        `🤖 AI Assistant processing query: "${query}" for shop ${shopId}`
      );

      // Analyze query intent
      const intent = this.analyzeQueryIntent(query);
      console.log(
        `📊 Query intent: ${intent.type} (confidence: ${intent.confidence})`
      );
  ...
  async handleAnalyticsQuery(query, shopId, intent) {
    ...
    return {
      type: 'analytics',
      message:
        'I can help you analyze cycle times, revenue, parts efficiency, and more. What specific metric would you like to see?',
    };
  }
  ...
  async handleWorkflowQuery(query, shopId, intent) {
    ...
    return {
      type: 'workflow',
      message:
        'I can help you track repair order statuses, parts workflows, and identify bottlenecks. What workflow would you like to check?',
    };
  }
  ...
  async handleFinancialQuery(query, shopId, intent) {
    return {
      type: 'financial',
      message:
        "Financial analysis features are coming soon! I'll be able to help with revenue, profit margins, and cost analysis.",
    };
  }
  ...
  generateSearchInsights(results, searchType) {
    ...
    if (searchType === 'vehicles') {
      ...
      const topMake = Object.keys(makeCount).reduce((a, b) =>
        makeCount[a] > makeCount[b] ? a : b
      );
      insights.push(
        `Most common vehicle: ${topMake} (${makeCount[topMake]} vehicles)`
      );
    }
  ...
  loadCollisionRepairKnowledge() {
    return {
      ...
      'cycle time':
        'Cycle time measures how long a vehicle spends in your shop from drop-off to completion.',
      supplement:
        "A supplement is additional work discovered during repair that wasn't in the original estimate.",
      drp: 'DRP (Direct Repair Program) is a partnership agreement with insurance companies for preferred repairs.',
      'parts sourcing':
        'Parts sourcing involves finding the best vendor for parts based on price, quality, and delivery time.',
    };
  }
  ...
  findRelevantKnowledge(query) {
    ...
    return {
      answer:
        'I\'m your CollisionOS AI assistant! I can help you search for repair orders, analyze shop performance, track parts orders, and answer collision repair questions. Try asking me something like "Show me pending repair orders" or "What\'s our average cycle time?"',
      confidence: 0.7,
