...
async function seedCurrentMonthData() {
  try {
    ...
    if (existingJobs.length === 0) {
      ...
      const lastMonthJobs = [
        {
          ...
          checkInDate: new Date(
            lastMonth.start.getTime() + 2 * 24 * 60 * 60 * 1000
          ),
          actualDeliveryDate: new Date(
            lastMonth.end.getTime() - 3 * 24 * 60 * 60 * 1000
          ),
          cycleTime: 8.5,
          customerSatisfaction: 4.7,
          createdAt: new Date(
            lastMonth.start.getTime() + 1 * 24 * 60 * 60 * 1000
          ),
        },
        {
          shopId,
          ...
          partsAmount: 1200.0,
          checkInDate: new Date(
            lastMonth.start.getTime() + 10 * 24 * 60 * 60 * 1000
          ),
          actualDeliveryDate: new Date(
            lastMonth.end.getTime() - 5 * 24 * 60 * 60 * 1000
          ),
          cycleTime: 6.2,
          customerSatisfaction: 4.8,
          createdAt: new Date(
            lastMonth.start.getTime() + 8 * 24 * 60 * 60 * 1000
          ),
        },
      ];
      ...
      console.log(
        `✅ Created ${createdLastMonthJobs.length} last month jobs for comparison`
      );
    }
