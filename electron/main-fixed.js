// Fixed version with proper Electron module loading check
// This handles the case where require('electron') might return a path string

let electron;
try {
  electron = require('electron');

  // If electron is a string (path to binary), we're running in wrong context
  if (typeof electron === 'string') {
    console.error('Error: Cannot load Electron module - running in wrong context');
    console.error('This file should be run via Electron, not Node directly');
    console.error('Use: npx electron . or npm run electron');
    process.exit(1);
  }
} catch (error) {
