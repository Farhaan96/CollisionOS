...
async function testAIAssistant() {
  ...
  for (let i = 0; i < testQueries.length; i++) {
    ...
    try {
      ...
      const hasValidStructure =
        response && typeof response.message === 'string' && response.type;
      ...
      if (typeMatches) {
        ...
        if (response.insights && response.insights.length > 0) {
          console.log(
            `   💡 Insights: ${response.insights.length} insights generated`
          );
        }
        ...
        console.log(
          `   ❌ FAIL: Expected type "${test.expectedType}" but got "${response.type}"`
        );
      }
    } catch (error) {
  ...
  for (const test of intentTests) {
    ...
    console.log(
      `${matches ? '✅' : '❌'} "${test.query}" -> ${intent.type} (expected: ${test.expectedIntent})`
    );
  ...
  for (const test of entityTests) {
    ...
    console.log(
      `${matches ? '✅' : '❌'} "${test.query}" -> ${JSON.stringify(entities)}`
    );
    if (matches) passedTests++;
    totalTests++;
  }
  ...
  console.log('\n' + '='.repeat(60));
  console.log(
    `🎯 AI Assistant Test Results: ${passedTests}/${totalTests} tests passed`
  );
  ...
  if (successRate >= 90) {
    console.log(
      '🎉 EXCELLENT: AI Assistant is performing at excellent levels!'
    );
  } else if (successRate >= 75) {
    console.log(
      '✅ GOOD: AI Assistant is performing well with minor areas for improvement.'
    );
  } else if (successRate >= 50) {
    console.log(
      '⚠️  FAIR: AI Assistant needs some improvements to reach optimal performance.'
    );
  } else {
    console.log(
      '❌ NEEDS WORK: AI Assistant requires significant improvements.'
    );
  }
