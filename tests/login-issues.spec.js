...
test.describe('Login Issues', () => {
  test('check login page layout and input field issues', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      await page.screenshot({ path: `login-cutoff-${viewport.name}.png`, fullPage: true });
      ...
      if (formBox && buttonBox) {
        ...
        if (formBox.x + formBox.width > viewport.width) cutoffIssues.push('Form cut off on right');
        if (formBox.y < 0) cutoffIssues.push('Form cut off on top');
        if (formBox.y + formBox.height > viewport.height) cutoffIssues.push('Form cut off on bottom');
        
        if (buttonBox.x < 0) cutoffIssues.push('Button cut off on left');
        if (buttonBox.x + buttonBox.width > viewport.width) cutoffIssues.push('Button cut off on right');
        if (buttonBox.y < 0) cutoffIssues.push('Button cut off on top');
        if (buttonBox.y + buttonBox.height > viewport.height) cutoffIssues.push('Button cut off on bottom');
  ...
  test('password visibility toggle and input field behavior', async ({ page }) => {
    await page.goto('/');
    
    // Test password visibility toggle
    const passwordField = page.locator('input[placeholder="admin123"]');
    const eyeIcon = page.locator('svg').filter({ hasText: /visibility/ }).or(page.locator('[data-testid="VisibilityIcon"], [data-testid="VisibilityOffIcon"]'));
    ...
    if (!eyeClicked) {
      ...
      if (passwordBox) {
        await page.click(passwordBox.x + passwordBox.width - 30, passwordBox.y + passwordBox.height / 2);
      }
    }
