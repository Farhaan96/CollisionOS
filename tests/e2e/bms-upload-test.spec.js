...
test.describe('BMS File Upload Functionality Tests', () => {
  ...
  test('BMS Upload Page Accessibility Test', async ({ page }) => {
    ...
    const bmsPageLoaded = await page
      .locator('text=BMS File Import')
      .isVisible();
  ...
  test('BMS File Upload Interface Test', async ({ page }) => {
    ...
    if (isFileInputPresent) {
      ...
      const uploadArea = page.locator(
        'text=Drop your BMS file here or click to browse'
      );
      const isUploadAreaVisible = await uploadArea.isVisible();
      console.log(`📤 Upload area visible: ${isUploadAreaVisible}`);

      console.log(
        '✅ SUCCESS: File upload interface is present and configured correctly!'
      );
    } else {
  ...
  test('BMS File Upload Functionality Test', async ({ page }) => {
    console.log(
      '🔍 CRITICAL TEST: Testing actual BMS file upload functionality...'
    );
    ...
    page.on('response', response => {
      ...
      if (
        response.url().includes('/api/import/bms') ||
        ...
        console.log(
          `📡 BMS upload response: ${response.status()} ${response.statusText()}`
        );
      }
    });
    ...
    page.on('request', request => {
      if (
        request.url().includes('/api/import/bms') ||
        request.url().includes('upload') ||
        request.url().includes('bms')
      ) {
        console.log(
          `📡 BMS upload request: ${request.method()} ${request.url()}`
        );
      }
    });
    ...
    const selectedFileName = await page
      .locator('text=sample-bms-test.xml')
      .isVisible();
    ...
    if (isUploadButtonVisible && isUploadButtonEnabled) {
      ...
      const successMessage = await page
        .locator('.MuiAlert-standardSuccess, text=processed successfully')
        .isVisible();
    ...
    console.log(
      `Upload Button Available: ${isUploadButtonVisible ? '✅' : '❌'}`
    );
    console.log(
      `Upload Button Enabled: ${isUploadButtonEnabled ? '✅' : '❌'}`
    );

    if (uploadResponse) {
      console.log(
        `Server Response: ${uploadResponse.status} ${uploadResponse.statusText}`
      );
