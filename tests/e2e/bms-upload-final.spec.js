...
test.describe('BMS Upload Final Test', () => {
  test('should handle BMS file upload without XML parsing errors', async ({ page }) => {
    ...
    try {
      ...
      if (title && title.includes('BMS Files Dashboard')) {
        ...
        try {
          ...
          const dialogVisible = await page.locator('text=Drag and drop BMS XML files here, or click to browse').isVisible();
    ...
    console.log('✅ BMS Upload test completed successfully - no XML parsing errors found!');
  });
});
