...
test.describe('CollisionOS Dashboard Mobile Navigation', () => {
  ...
  test.describe('Mobile Dashboard Layout', () => {
    test('should display KPI cards in mobile-friendly layout', async ({
      page,
    }) => {
    ...
    test('should verify mobile touch targets are appropriately sized', async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
      ...
      const interactiveCards = page.locator(
        '.MuiCard-root[style*="cursor: pointer"], .MuiCard-root:hover'
      );
    ...
    test('should handle mobile scrolling for activity feed', async ({
      page,
    }) => {
  ...
  test.describe('Mobile Touch Interactions', () => {
    test('should support tap gestures on KPI cards', async ({ page }) => {
      ...
      if (await activeRepairsCard.isVisible()) {
        const cardElement = activeRepairsCard.locator(
    ...
    test('should support long press gestures for context menus', async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
    ...
    test('should support swipe gestures for navigation', async ({ page }) => {
      ...
      if (await techSection.isVisible()) {
        const sectionContainer = techSection.locator(
          'xpath=ancestor::*[contains(@class, "MuiPaper-root")]'
        );
  ...
  test.describe('Mobile Responsive Behavior', () => {
    ...
    test('should handle mobile navigation menu', async ({ page }) => {
      ...
      if ((await mobileMenuButton.count()) > 0) {
        ...
        const navigationDrawer = page.locator(
          '[role="dialog"], .MuiDrawer-root'
        );
        if ((await navigationDrawer.count()) > 0) {
          await expect(navigationDrawer.first()).toBeVisible();

          // Should contain navigation links
          const navLinks = navigationDrawer.locator(
            'text=Dashboard, text=Production, text=Customers'
          );
  ...
  test.describe('Mobile Performance Tests', () => {
    test('should load quickly on mobile network conditions', async ({
      page,
    }) => {
  ...
  test.describe('Mobile Edge Cases', () => {
    ...
    test('should handle mobile app-like navigation patterns', async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
      ...
      const dashboardContainer = page
        .locator('[data-testid="dashboard"], main, .MuiContainer-root')
        .first();
      ...
      if (boundingBox) {
        ...
        await expect(
          page.locator('text=Auto Body Shop Dashboard')
        ).toBeVisible();
      }
    });

    test('should maintain state during mobile interruptions', async ({
      page,
    }) => {
      await page.waitForTimeout(2000);
