...
test.describe('CollisionOS Dashboard Accessibility Navigation', () => {
  ...
  test.describe('Keyboard Navigation', () => {
    ...
    test('should support Enter key activation on focusable elements', async ({ page }) => {
      await page.waitForTimeout(2000);
        ...
        if (await focusedElement.count() > 0) {
          ...
          if (tagName === 'BUTTON' || tagName === 'A' || role === 'button' || role === 'link') {
            ...
            if (newUrl !== initialUrl) {
              console.log(`Enter key navigated from ${initialUrl} to ${newUrl}`);
    ...
    test('should support arrow key navigation in grids and lists', async ({ page }) => {
      ...
      for (const arrowKey of arrowKeyTests) {
        const beforeFocus = await page.evaluate(() => document.activeElement?.textContent?.slice(0, 30));
        
        await page.keyboard.press(arrowKey);
        await page.waitForTimeout(200);
        
        const afterFocus = await page.evaluate(() => document.activeElement?.textContent?.slice(0, 30));
    ...
    test('should support Escape key to close modals and cancel actions', async ({ page }) => {
      await page.waitForTimeout(2000);
      ...
      if (triggerCount > 0) {
        ...
        const modal = page.locator('[role="dialog"], .MuiDialog-root, .MuiModal-root');
  ...
  test.describe('Screen Reader Support', () => {
    ...
    test('should provide meaningful alternative text and labels', async ({ page }) => {
      await page.waitForTimeout(2000);
    ...
    test('should announce dynamic content changes', async ({ page }) => {
      ...
      if (await interactiveElement.isVisible()) {
        ...
        const updatedRegions = page.locator('[aria-live]:not(:empty), [role="status"]:not(:empty)');
        const updatedCount = await updatedRegions.count();
        
        console.log(`Found ${updatedCount} updated live regions after interaction`);
      }
    });
  });
  ...
  test.describe('Visual Accessibility', () => {
    test('should have sufficient color contrast ratios', async ({ page }) => {
      ...
      const textElements = page.locator('.MuiTypography-root, .MuiButton-root, .MuiChip-root');
    ...
    test('should scale properly with increased font sizes', async ({ page }) => {
  ...
  test.describe('Focus Management', () => {
    ...
    test('should provide visible focus indicators', async ({ page }) => {
      ...
      const elementsWithFocus = focusTests.filter(test => test.hasFocusIndicator);
