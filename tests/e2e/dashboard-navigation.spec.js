...
test.describe('CollisionOS Dashboard Navigation System', () => {
  ...
  test.describe('1. KPI Card Navigation Tests', () => {
    test('should verify all 12+ KPI cards are present and clickable', async ({ page }) => {
      // Wait for dashboard to fully load
      await page.waitForTimeout(2000);
      
      // Define expected KPI cards with their text and navigation targets
      const expectedKPIs = [
        { title: 'Active Repairs', value: '24', shouldNavigate: true, expectedUrl: '/production' },
        { title: 'Today\'s Deliveries', value: '3/8', shouldNavigate: true, expectedUrl: '/deliveries' },
        { title: 'Monthly Revenue', value: '$249K', shouldNavigate: true, expectedUrl: '/analytics' },
        { title: 'Parts Inventory', value: '1247', shouldNavigate: true, expectedUrl: '/parts' },
        { title: 'Technician Utilization', value: '87.5%', shouldNavigate: true, expectedUrl: '/technicians' },
        { title: 'Average Cycle Time', value: '5.8 days', shouldNavigate: true, expectedUrl: '/analytics' },
        { title: 'Job Completion Rate', value: '94.2%', shouldNavigate: true, expectedUrl: '/production' },
        { title: 'Customer Satisfaction', value: '4.7/5.0', shouldNavigate: true, expectedUrl: '/customers' },
        { title: 'Insurance Claims', value: '142', shouldNavigate: true, expectedUrl: '/insurance' },
        { title: 'Shop Capacity', value: '24/28', shouldNavigate: true, expectedUrl: '/capacity' },
        { title: 'Quality Score', value: '96.8%', shouldNavigate: true, expectedUrl: '/quality' },
        { title: 'Average Ticket', value: '$3,247', shouldNavigate: true, expectedUrl: '/analytics' }
      ];
      ...
      for (const kpi of expectedKPIs) {
        ...
        const cardElement = kpiCard.locator('xpath=ancestor::*[contains(@class, "MuiCard-root")]').first();
    ...
    test('should navigate to correct URLs when KPI cards are clicked', async ({ page }) => {
      await page.waitForTimeout(2000);

      // Test Active Repairs navigation
      const activeRepairsCard = page.locator('text=Active Repairs').locator('xpath=ancestor::*[contains(@class, "MuiCard-root")]').first();
    ...
    test('should pass correct URL parameters for filtered views', async ({ page }) => {
      await page.waitForTimeout(2000);

      // Test Parts Inventory navigation with expected filtering
      const partsCard = page.locator('text=Parts Inventory').locator('xpath=ancestor::*[contains(@class, "MuiCard-root")]').first();
    ...
    test('should verify trend indicators are interactive', async ({ page }) => {
      ...
      const trendIcons = page.locator('[data-testid*="trending"], .MuiSvgIcon-root[data-testid*="Trending"]');
      ...
      if (trendCount > 0) {
        ...
        for (let i = 0; i < Math.min(trendCount, 3); i++) {
          ...
          const iconColor = await trendIcon.evaluate(el => 
  ...
  test.describe('2. Activity Feed Navigation Tests', () => {
    ...
    test('should navigate to correct context when activity items are clicked', async ({ page }) => {
      await page.waitForTimeout(3000);

      // Find job completion activity items
      const jobCompletedItem = page.locator('text=Job CR-2024-008 completed').first();
      
      if (await jobCompletedItem.isVisible()) {
        const parentItem = jobCompletedItem.locator('xpath=ancestor::*[contains(@class, "MuiListItem-root")]');
    ...
    test('should handle parts arrival navigation', async ({ page }) => {
      ...
      if (await partsArrivedItem.isVisible()) {
        const parentItem = partsArrivedItem.locator('xpath=ancestor::*[contains(@class, "MuiListItem-root")]');
        await parentItem.click();
    ...
    test('should handle quality alert navigation', async ({ page }) => {
      ...
      if (await qualityIssueItem.isVisible()) {
        const parentItem = qualityIssueItem.locator('xpath=ancestor::*[contains(@class, "MuiListItem-root")]');
        await parentItem.click();
  ...
  test.describe('3. Technician Performance Tests', () => {
    test('should verify technician cards are interactive', async ({ page }) => {
      ...
      const techCards = page.locator('text=Mike Rodriguez, text=Sarah Chen, text=James Wilson, text=Lisa Garcia');
    ...
    test('should navigate to individual technician pages', async ({ page }) => {
      ...
      if (await mikeCard.isVisible()) {
        const parentElement = mikeCard.locator('xpath=ancestor::*[contains(@style, "cursor") or contains(@class, "clickable") or @role="button"]');
    ...
    test('should verify technician performance metrics', async ({ page }) => {
      ...
      const utilizationMetrics = page.locator('text=94%, text=89%, text=82%, text=85%');
  ...
  test.describe('4. Alert Navigation Tests', () => {
    ...
    test('should handle parts delay alert navigation', async ({ page }) => {
      ...
      const partsDelayAlert = page.locator('text*=Parts Delay Alert, text*=Critical parts').first();
      
      if (await partsDelayAlert.isVisible()) {
        const parentAlert = partsDelayAlert.locator('xpath=ancestor::*[contains(@class, "MuiAlert-root")]');
    ...
    test('should handle capacity warning navigation', async ({ page }) => {
      ...
      const capacityWarning = page.locator('text*=Capacity Warning, text*=96% capacity').first();
      
      if (await capacityWarning.isVisible()) {
        const parentAlert = capacityWarning.locator('xpath=ancestor::*[contains(@class, "MuiAlert-root")]');
        const actionButton = parentAlert.locator('button');
    ...
    test('should handle insurance follow-up alerts', async ({ page }) => {
      ...
      const insuranceAlert = page.locator('text*=Insurance Follow-up, text*=claims pending').first();
      
      if (await insuranceAlert.isVisible()) {
        const parentAlert = insuranceAlert.locator('xpath=ancestor::*[contains(@class, "MuiAlert-root")]');
        const actionButton = parentAlert.locator('button');
  ...
  test.describe('5. Visual Interaction Tests', () => {
    test('should verify hover effects on interactive elements', async ({ page }) => {
      await page.waitForTimeout(2000);
      ...
      const initialTransform = await firstKPICard.evaluate(el => 
      ...
      const hoveredTransform = await firstKPICard.evaluate(el => 
        window.getComputedStyle(el).transform
      );
    ...
    test('should verify cursor changes to pointer on clickable items', async ({ page }) => {
      await page.waitForTimeout(2000);
    ...
    test('should validate mobile touch targets are appropriate size', async ({ page }) => {
      // Set mobile viewport
  ...
  test.describe('6. Cross-Page Integration Tests', () => {
    test('should maintain user context during navigation', async ({ page }) => {
      ...
      const userGreeting = page.locator('text*=Welcome, text*=admin, text*=Manager');
    ...
    test('should verify filtered views load with correct data', async ({ page }) => {
      await page.waitForTimeout(2000);
      ...
      if (await activeRepairsCard.isVisible()) {
        const parentCard = activeRepairsCard.locator('xpath=ancestor::*[contains(@class, "MuiCard-root")]');
        ...
        if (currentUrl.includes('production')) {
          ...
          const activeJobs = page.locator('text*=J-2024, text*=In Progress, text*=Active');
    ...
    test('should verify back button functionality maintains dashboard state', async ({ page }) => {
      await page.waitForTimeout(2000);
  ...
  test.describe('7. Error Handling Tests', () => {
    test('should handle navigation with invalid parameters gracefully', async ({ page }) => {
    ...
    test('should verify graceful handling of missing data', async ({ page }) => {
      await page.waitForTimeout(2000);

      // Check that dashboard handles missing/empty data gracefully
      // Look for fallback content or loading states
      const errorMessages = page.locator('text*=Error, text*=Failed, text*=Unable');
      ...
      const fallbackContent = page.locator('text*=No data, text*=Loading, .MuiSkeleton-root');
  ...
  test.describe('8. Accessibility Tests', () => {
    test('should verify interactive elements have proper ARIA attributes', async ({ page }) => {
      await page.waitForTimeout(2000);
