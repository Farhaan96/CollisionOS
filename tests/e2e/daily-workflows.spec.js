...
test.describe('CollisionOS Daily Workflow Tests', () => {
  ...
  test.describe('Service Advisor Daily Workflow', () => {
    test('Complete customer intake and estimate workflow', async ({ page }) => {
      ...
      const customerTable = page.locator('table, [role="grid"], .MuiDataGrid-root');
      await expect(customerTable).toBeVisible({ timeout: 10000 });
      
      // Check for add customer button
      const addButton = page.locator('button:has-text("Add"), button:has-text("New Customer"), button:has-text("Create")').first();
      if (await addButton.isVisible()) {
        // Click add customer
        await addButton.click();
        
        // Fill customer form if modal/form appears
        const formContainer = page.locator('form, [role="dialog"], .MuiModal-root, .MuiDialog-root').first();
        if (await formContainer.isVisible({ timeout: 5000 }).catch(() => false)) {
          ...
          // Save customer
          const saveButton = page.locator('button:has-text("Save"), button:has-text("Submit"), button:has-text("Create")').last();
      ...
      const productionBoard = page.locator('.production-board, .kanban-board, [class*="production"], [class*="board"]').first();
      await expect(productionBoard).toBeVisible({ timeout: 10000 });
      
      // Check for different workflow stages
      const stages = ['Check-In', 'Disassembly', 'Parts', 'Repair', 'Paint', 'Reassembly', 'Detail', 'Quality Control'];
  ...
  test.describe('Technician Daily Workflow', () => {
    test('Update job status and request parts', async ({ page }) => {
      ...
      // Look for job cards
      const jobCards = page.locator('[class*="card"], [class*="job"], [class*="repair-order"]');
      ...
      if (jobCount > 0) {
        ...
        const detailView = page.locator('[role="dialog"], .MuiModal-root, .job-detail, .repair-detail').first();
        if (await detailView.isVisible({ timeout: 3000 }).catch(() => false)) {
          // Look for status update options
          const statusDropdown = page.locator('select, [role="combobox"], [class*="select"]').first();
      ...
      const partsInterface = page.locator('[class*="parts"], table, .MuiDataGrid-root').first();
      await expect(partsInterface).toBeVisible({ timeout: 10000 });
      
      // Check for parts request button
      const requestButton = page.locator('button:has-text("Request"), button:has-text("Order"), button:has-text("Add Part")').first();
  ...
  test.describe('Manager Daily Workflow', () => {
    test('Review dashboard metrics and generate reports', async ({ page }) => {
      ...
      // Check for KPI cards
      const kpiCards = page.locator('[class*="card"], [class*="metric"], [class*="kpi"]');
      const kpiCount = await kpiCards.count();
      expect(kpiCount).toBeGreaterThan(0);
      
      // Look for specific metrics
      const metrics = ['Revenue', 'Jobs', 'Cycle Time', 'Efficiency', 'Customer', 'Parts'];
      ...
      if (await reportsLink.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        const reportTypes = ['Financial', 'Production', 'Customer', 'Parts', 'Technician'];
  ...
  test.describe('Parts Manager Daily Workflow', () => {
    test('Manage inventory and process orders', async ({ page }) => {
      ...
      const partsTable = page.locator('table, [role="grid"], .MuiDataGrid-root').first();
      await expect(partsTable).toBeVisible({ timeout: 10000 });
      
      // Check for inventory management features
      const inventoryFeatures = ['Add Part', 'Stock', 'Order', 'Vendor', 'Receive'];
      ...
      const vendorTab = page.locator('text=/vendor/i, [role="tab"]:has-text("Vendor")').first();
  ...
  test.describe('Admin System Configuration', () => {
    test('Manage users and system settings', async ({ page }) => {
      ...
      const settingsLink = page.locator('text=/settings/i, text=/admin/i, text=/configuration/i').first();
      if (await settingsLink.isVisible({ timeout: 5000 }).catch(() => false)) {
        await settingsLink.click();
        await page.waitForLoadState('networkidle');
        
        // Check for user management
        const userManagement = page.locator('text=/user/i, text=/staff/i, text=/employee/i').first();
        ...
        const configOptions = ['General', 'Security', 'Database', 'Integration', 'Backup'];
        for (const option of configOptions) {
          const configElement = page.locator(`text=/${option}/i`).first();
          if (await configElement.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const themeToggle = page.locator('[aria-label*="theme" i], [title*="theme" i], button:has-text("Dark"), button:has-text("Light")').first();
  ...
  test.describe('Cross-functional Workflows', () => {
    test('Complete repair order from intake to delivery', async ({ page }) => {
      ...
      const qcLink = page.locator('text=/quality/i, text=/qc/i, text=/inspection/i').first();
    ...
    test('Handle insurance claim workflow', async ({ page }) => {
      ...
      const bmsLink = page.locator('text=/bms/i, text=/insurance/i, text=/claim/i').first();
  ...
  test.describe('Performance and Reliability', () => {
    test('System handles concurrent operations', async ({ page }) => {
      ...
      const dashboardContent = page.locator('[class*="dashboard"], [class*="metric"], main').first();
  ...
  test.describe('Mobile Responsiveness', () => {
    test('Critical workflows function on mobile devices', async ({ page }) => {
      ...
      const mobileMenu = page.locator('[aria-label*="menu" i], button:has-text("☰"), .MuiIconButton-root').first();
