...
const createMockFile = (name = 'test.xml', content = '<xml>test</xml>', type = 'text/xml') => {
...
describe('BMSFileUpload Component', () => {
  ...
  describe('Initial Rendering', () => {
    it('should render upload area correctly', () => {
      ...
      expect(screen.getByText(/Drag and drop BMS XML files here/)).toBeInTheDocument();
    ...
    it('should show support information', () => {
      ...
      expect(screen.getByText(/Supports Mitchell Estimating BMS XML files/)).toBeInTheDocument();
  ...
  describe('File Selection', () => {
    it('should handle file selection through input', async () => {
      ...
      renderWithTheme(<BMSFileUpload onUploadComplete={mockOnUploadComplete} />);
    ...
    it('should filter non-XML files', async () => {
      ...
      const uploadArea = screen.getByText('Upload BMS Files').closest('[role="button"]') || 
      ...
      if (uploadArea) {
        ...
        await waitFor(() => {
          expect(screen.getByText(/Please upload valid BMS XML files/)).toBeInTheDocument();
        });
      }
    });
  });
  ...
  describe('Upload Progress and Feedback', () => {
    it('should show progress during upload', async () => {
      ...
      bmsService.uploadBMSFile.mockImplementation(() => 
        new Promise(resolve => 
          setTimeout(() => resolve({ success: true, data: {}, message: 'Success' }), 1000)
        )
      );
  ...
  describe('Error Handling', () => {
    it('should display error messages on upload failure', async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockOnError).toHaveBeenCalledWith('Invalid BMS format', mockFile);
      });
    });
  ...
  describe('File Management', () => {
    ...
    it('should allow file removal', async () => {
      ...
      const removeButton = screen.getByTitle('Remove file') || screen.getByLabelText('Remove file');
    ...
    it('should allow file details expansion', async () => {
      ...
      await waitFor(() => {
        const expandButton = screen.getByTitle(/Expand details/) || screen.getByTitle(/Collapse details/);
