...
describe('ExecutiveKPICard', () => {
  ...
  it('handles click events', () => {
    ...
    const card = screen.getByText('REVENUE').closest('div[style*="cursor: pointer"]');
    if (card) {
  ...
  it('handles different sizes correctly', () => {
    ...
    expect(container.querySelector('[style*="min-height: 120px"]')).toBeInTheDocument();
    ...
    expect(container.querySelector('[style*="min-height: 200px"]')).toBeInTheDocument();
  });
  ...
  it('handles medium size as default', () => {
    const { container } = renderWithTheme(<ExecutiveKPICard {...defaultProps} />);
    
    // Should have medium size styling (160px min-height)
    expect(container.querySelector('[style*="min-height: 160px"]')).toBeInTheDocument();
  });
});