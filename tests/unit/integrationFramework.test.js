...
const { IntegrationClient, IntegrationManager } = require('../../server/services/integrationFramework');
...
describe('Integration Framework', () => {
  ...
  describe('IntegrationClient', () => {
    ...
    describe('Health Check', () => {
      test('should return healthy status', async () => {
        ...
        expect(client.get).toHaveBeenCalledWith('/health', {}, { timeout: 5000 });
      });

      test('should return unhealthy status on error', async () => {
        client.get = jest.fn().mockRejectedValue(new Error('Connection failed'));
  ...
  describe('IntegrationManager', () => {
    ...
    test('should handle webhook with signature verification', async () => {
      ...
      const payload = { data: 'test' };
      const result = await manager.handleWebhook('test', 'event', payload, 'signature');
    ...
    test('should reject webhook with invalid signature', async () => {
      ...
      const payload = { data: 'test' };
      const result = await manager.handleWebhook('test', 'event', payload, 'invalid-signature');
    ...
    test('should ignore webhook with no handler', async () => {
      ...
      const payload = { data: 'test' };
      const result = await manager.handleWebhook('test', 'unknown_event', payload);
      
      expect(result.status).toBe('ignored');
    });

    test('should health check all providers', async () => {
      const mockClient1 = { healthCheck: jest.fn().mockResolvedValue({ status: 'healthy' }) };
      const mockClient2 = { healthCheck: jest.fn().mockResolvedValue({ status: 'healthy' }) };
