...
describe('API Performance Tests', () => {
  ...
  describe('Concurrent Request Performance', () => {
    test('should handle multiple concurrent GET requests', async () => {
      ...
      for (let i = 0; i < concurrentRequests; i++) {
        promises.push(request(app).get('/api/customers').expect(200));
      }
      ...
      console.log(
        `${concurrentRequests} concurrent requests completed in ${totalTime}ms (avg: ${averageTime.toFixed(2)}ms)`
      );
    ...
    test('should handle sustained load', async () => {
      ...
      for (let batch = 0; batch < batchCount; batch++) {
        ...
        for (let i = 0; i < requestsPerBatch; i++) {
          batchPromises.push(request(app).get('/api/customers').expect(200));
        }
        ...
        console.log(
          `Batch ${batch + 1}: ${requestsPerBatch} requests in ${batchTime}ms (avg: ${(batchTime / requestsPerBatch).toFixed(2)}ms)`
        );
      ...
      const avgBatchTime =
        batchResults.reduce((sum, batch) => sum + batch.averageTime, 0) /
        batchCount;
      const maxBatchTime = Math.max(
        ...batchResults.map(batch => batch.averageTime)
      );
      const minBatchTime = Math.min(
        ...batchResults.map(batch => batch.averageTime)
      );

      console.log(
        `Load test summary: avg=${avgBatchTime.toFixed(2)}ms, min=${minBatchTime.toFixed(2)}ms, max=${maxBatchTime.toFixed(2)}ms`
      );
  ...
  describe('Database Performance Simulation', () => {
    test('should handle simulated slow database queries', async () => {
      ...
      const response = await request(app).get('/api/slow-query').expect(200);
    ...
    test('should handle concurrent slow queries', async () => {
      ...
      for (let i = 0; i < concurrentSlowRequests; i++) {
        slowPromises.push(request(app).get('/api/slow-query').expect(200));
      }
      ...
      console.log(
        `${concurrentSlowRequests} concurrent slow queries completed in ${totalTime}ms`
      );
