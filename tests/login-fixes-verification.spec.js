...
test.describe('Login Fixes Verification', () => {
  test('verify password toggle works and input fields display correctly', async ({ page }) => {
    await page.goto('/');
    
    // Test password visibility toggle
    const passwordField = page.locator('input[placeholder="admin123"]');
    const eyeContainer = page.locator('div').filter({ has: page.locator('svg[data-testid="VisibilityIcon"], svg[data-testid="VisibilityOffIcon"]') });
  ...
  test('login page responsiveness on different screen sizes', async ({ page }) => {
    await page.goto('/');
    ...
    for (const viewport of viewports) {
      ...
      await page.screenshot({ path: `login-responsive-${viewport.name}.png`, fullPage: true });
