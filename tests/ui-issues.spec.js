...
test.describe('CollisionOS UI Issues', () => {
  test('capture login page screenshot and check branding', async ({ page }) => {
    ...
    if ((await precisionText.count()) > 0) {
      console.log(
        'Found "Precision Autobody" text that should be "CollisionOS"'
      );
    }
  ...
  test('capture dashboard page and check for text overflow', async ({
    page,
  }) => {
    await page.goto('/');
    ...
    const dashboardContent = page.locator(
      '[data-testid="dashboard"], .dashboard, h1:has-text("Dashboard"), h2:has-text("Dashboard")'
    );
