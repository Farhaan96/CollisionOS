...
test.describe('CollisionOS Comprehensive UI Testing', () => {
  ...
  test('Phase 3: Navigation and Routing', async ({ page }) => {
    ...
    const menuItems = ['Dashboard', 'BMS Import', 'BMS Dashboard', 'Customers', 'Production', 'Parts', 'Technician', 'QC', 'Reports'];
  ...
  test('Phase 4: Form Functionality - Customer Management', async ({ page }) => {
    ...
    try {
      ...
      const hasNameField = await page.locator('input[name*="name"], input[placeholder*="name"], input[label*="Name"]').count();
      const hasEmailField = await page.locator('input[type="email"], input[name*="email"], input[placeholder*="email"]').count();
      const hasPhoneField = await page.locator('input[type="tel"], input[name*="phone"], input[placeholder*="phone"]').count();
      
      console.log(`  ✓ Form fields found - Name: ${hasNameField}, Email: ${hasEmailField}, Phone: ${hasPhoneField}`);
      
      // Try to close modal if open
      const closeButtons = await page.locator('button:has-text("Cancel"), button:has-text("Close"), [aria-label="close"]').count();
      if (closeButtons > 0) {
        await page.click('button:has-text("Cancel"), button:has-text("Close"), [aria-label="close"]');
        console.log('  ✓ Modal closed successfully');
      }
    } catch (error) {
      console.log('  ⚠ Add Customer functionality may not be available or has different UI');
    }
  });
  
  test('Phase 5: Interactive Elements - Buttons and Controls', async ({ page }) => {
    ...
    try {
      const refreshButton = page.locator('button:has-text("Refresh"), button[aria-label*="refresh"], button[title*="refresh"]');
    ...
    const searchInputs = await page.locator('input[type="search"], input[placeholder*="search"], input[placeholder*="filter"]').count();
    const dropdowns = await page.locator('select, [role="combobox"], [role="listbox"]').count();
    
    console.log(`  ✓ Found ${searchInputs} search inputs and ${dropdowns} dropdown controls`);
  });
  ...
  test('Phase 6: Interface Design - Visual Elements', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      const navVisible = await page.locator('nav, [role="navigation"], .navbar, .menu').count();
      console.log(`  ✓ Navigation elements visible: ${navVisible > 0}`);
      
      // Test if main content is visible
      const mainContent = await page.locator('main, [role="main"], .content, #main').count();
  ...
  test('Phase 6: Material-UI Component Rendering', async ({ page }) => {
    ...
    for (const pageName of pages) {
      ...
      const cards = await page.locator('.MuiCard-root, [class*="MuiCard"]').count();
      const buttons = await page.locator('.MuiButton-root, [class*="MuiButton"]').count();
      const textFields = await page.locator('.MuiTextField-root, [class*="MuiTextField"]').count();
      const chips = await page.locator('.MuiChip-root, [class*="MuiChip"]').count();
      const tables = await page.locator('.MuiTable-root, [class*="MuiTable"], table').count();
      ...
      const muiThemed = await page.locator('[class*="Mui"], [class*="MuiThemeProvider"]').count();
  ...
  test('Phase 6: Typography and Color Scheme', async ({ page }) => {
    ...
    console.log(`  ✓ Typography elements - Headings: ${headings}, Body text: ${bodyText}`);
    
    // Check for consistent color scheme by looking for common CSS classes
    const bodyHTML = await page.innerHTML('body');
    const hasColorClasses = bodyHTML.includes('color') || bodyHTML.includes('primary') || bodyHTML.includes('secondary');
  ...
  test('Phase 6: Layout Alignment and Spacing', async ({ page }) => {
    ...
    // Test grid layouts
    const gridElements = await page.locator('[class*="Grid"], [class*="grid"], .container, .row').count();
    console.log(`  ✓ Grid/layout elements: ${gridElements}`);
    
    // Test consistent spacing
    const spacedElements = await page.locator('[class*="margin"], [class*="padding"], [class*="spacing"]').count();
    console.log(`  ✓ Spaced elements: ${spacedElements}`);
    
    // Check for proper alignment
    const alignedElements = await page.locator('[class*="align"], [class*="justify"], [class*="center"]').count();
  ...
  test('Performance and Load Testing', async ({ page }) => {
    ...
    for (const pageName of pages) {
      ...
      const loadingIndicators = await page.locator('[class*="loading"], [class*="spinner"], .MuiCircularProgress-root').count();
