...
export class MobileAPIClient {
    ...
    for (const item of queue) {
      try {
        ...
        if (item.config.retries < this.options.retries) {
          ...
          console.error('Failed to sync offline request after retries:', item.id);
        }
      }
    }
  ...
  async getJobStatus(jobId) {
    const response = await this.client.get(`/api/mobile/customer/jobs/${jobId}/status`);
    return response.data;
  }
  
  async getJobTimeline(jobId) {
    const response = await this.client.get(`/api/mobile/customer/jobs/${jobId}/timeline`);
    return response.data;
  }
  ...
  async getEstimate(estimateId) {
    const response = await this.client.get(`/api/mobile/customer/estimates/${estimateId}`);
    return response.data;
  }
  ...
  async getMessages(customerId) {
    const response = await this.client.get(`/api/mobile/customer/messages?customerId=${customerId}`);
    return response.data;
  }
  
  async sendMessage(message) {
    const response = await this.client.post('/api/mobile/customer/messages', message);
    return response.data;
  }
  
  // Technician API methods
  async getTechnicianTasks(technicianId) {
    const response = await this.client.get(`/api/mobile/technician/tasks?technicianId=${technicianId}`);
    return response.data;
  }
  
  async updateTaskStatus(taskId, status, notes = '') {
    const response = await this.client.put(`/api/mobile/technician/tasks/${taskId}/status`, {
      status,
      notes
    });
    return response.data;
  ...
  async startTimeEntry(taskId, jobId) {
    const response = await this.client.post('/api/mobile/technician/time-entries', {
      taskId,
      jobId,
  ...
  async endTimeEntry(timeEntryId, notes = '') {
    const response = await this.client.put(`/api/mobile/technician/time-entries/${timeEntryId}`, {
  ...
  async scanJobQR(qrCode) {
    const response = await this.client.get(`/api/mobile/technician/jobs/scan/${encodeURIComponent(qrCode)}`);
    return response.data;
  }
