...
import { useNotification as useNotificationContext, NOTIFICATION_TYPES, NOTIFICATION_PRIORITIES } from '../components/Notifications/NotificationProvider';
...
export const useNotification = () => {
  ...
  const withNotification = useCallback(async (
    operation,
    {
  ...
  // Batch notifications
  const batch = useCallback((notifications) => {
  ...
  // Dismiss by pattern
  const dismissBy = useCallback((predicate) => {
  ...
  const queue = useCallback((notification, delay = 0) => {
