...
export const useLoadingState = (config = {}) => {
  ...
  // Derived state
  const isLoading =
    state === LOADING_STATES.LOADING || state === LOADING_STATES.RETRYING;
    ...
    async (operation, options = {}) => {
      ...
      try {
        ...
        const remainingTime = Math.max(
          0,
          operationConfig.minimumLoadTime - elapsed
        );
  ...
  const setErrorManually = useCallback(
    err => {
  ...
  return {
    ...
    withProgressTracking: useCallback(
      async (operation, estimatedDuration = 5000) => {
    ...
    withRetry: useCallback(
      async (operation, maxAttempts = 3) => {
        return execute(operation, {
