...
const BMSDataValidationUI = ({
  ...
  const [overrideDialog, setOverrideDialog] = useState({ open: false, issue: null });
  const [autoFixDialog, setAutoFixDialog] = useState({ open: false, fixes: [] });
  ...
  const validationStats = useMemo(() => {
    ...
    Object.entries(validationResults).forEach(([section, result]) => {
      if (result.errors) {
        ...
        allIssues.push(...result.errors.map(e => ({ ...e, section, type: 'error' })));
      }
      if (result.warnings) {
        totalWarnings += result.warnings.length;
        allIssues.push(...result.warnings.map(w => ({ ...w, section, type: 'warning' })));
      }
      if (result.fieldValidations) {
        Object.entries(result.fieldValidations).forEach(([field, validation]) => {
          totalFields++;
    ...
    return {
      ...
      totalFields,
      validFields,
      validationScore: totalFields > 0 ? Math.round((validFields / totalFields) * 100) : 100,
        ...
        <Box sx={{ mb: 3 }}>
          <LinearProgress
            ...
            sx={{
              height: 12,
              borderRadius: 6,
              backgroundColor: alpha(getValidationScoreColor(validationStats.validationScore), 0.2),
          ...
          <Stack direction="row" spacing={2}>
            {validationStats.totalIssues > 0 && (
              <Button
                ...
                onClick={() => setAutoFixDialog({ open: true, fixes: validationStats.allIssues })}
          ...
          <Box sx={{ p: 3, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.1)}` }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                >
                  <PlaylistAddCheck sx={{ color: theme.palette.secondary.main, fontSize: 24 }} />
                </Box>
                <Box>
              ...
              <Badge badgeContent={filteredValidations.length} color="primary">
                <Chip 
                  ...
                  color={validationStats.validFields === validationStats.totalFields ? 'success' : 'warning'}
                  variant="filled"
                />
              </Badge>
            </Box>
          ...
          <Box sx={{ p: 3 }}>
            <List sx={{ p: 0 }}>
              {filteredValidations.map((validation, index) => (
                <React.Fragment key={`${validation.section}-${validation.field}`}>
                  <ListItem
                    sx={{
                      p: 2.5,
                      borderRadius: '12px',
                    ...
                    {!validation.isValid && allowOverrides && (
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        <Tooltip title="Override validation">
                          <IconButton
                            size="small"
                            onClick={() => handleOverrideIssue({ field: validation.field, message: validation.message })}
                            sx={{
                  ...
                  {index < filteredValidations.length - 1 && <Divider sx={{ opacity: 0.3 }} />}
                </React.Fragment>
              ))}
            </List>
            ...
            {filteredValidations.length === 0 && (
              ...
              >
                <CheckCircle sx={{ color: theme.palette.success.main, fontSize: 48, mb: 2 }} />
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, width: '100%' }}>
              ...
              <Box sx={{ flex: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  {result.summary?.message || (hasIssues ? `${(result.errors?.length || 0) + (result.warnings?.length || 0)} issues found` : 'All validations passed')}
                </Typography>
              </Box>
    ...
    <Box sx={{ width: '100%' }}>
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography variant="body2">
            Overriding this validation will allow the import to proceed despite the identified issue. 
            Please ensure you understand the implications before proceeding.
          </Typography>
        </DialogContent>

        <DialogActions sx={{ p: 3 }}>
          <Button onClick={() => setOverrideDialog({ open: false, issue: null })}>
            Cancel
          </Button>
