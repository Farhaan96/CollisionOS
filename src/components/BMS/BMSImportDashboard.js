...
const BMSImportDashboard = ({ 
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'validating': return theme.palette.info.main;
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'completed': return <CloudDone />;
      case 'error': return <Error />;
      case 'validating': return <Assessment />;
      default: return <Info />;
    }
  };
        ...
        >
          <CardContent sx={{ p: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Box>
                  <Typography variant="h5" sx={{ fontWeight: 700, mb: 0.5 }}>
                    Import Status: {importStatus.charAt(0).toUpperCase() + importStatus.slice(1)}
                  </Typography>
            ...
            <Grid container spacing={3}>
              {[
                { label: 'Files Processed', value: processingStats.filesProcessed || 0, icon: DataObject },
                { label: 'Validation Checks', value: validationResults.length || 0, icon: TaskAlt },
                { label: 'Issues Found', value: validationResults.filter(v => v.errors?.length > 0).length || 0, icon: BugReport },
                { label: 'Processing Time', value: `${processingStats.processingTime || 0}s`, icon: Speed }
              ].map((stat, index) => (
                <Grid item xs={6} sm={3} key={stat.label}>
                  <Box sx={{ textAlign: 'center' }}>
                    ...
                    >
                      <stat.icon sx={{ color: theme.palette.primary.main, fontSize: 20 }} />
                    </Box>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  >
                    <Button
                      ...
                      endIcon={expandedSections.has('validation') ? <ExpandLess /> : <ExpandMore />}
                      sx={{
                        borderRadius: '12px',
                        textTransform: 'none'
                      }}
                      ...
                      <React.Fragment key={index}>
                        <ListItem
                          sx={{
                            ...
                            border: `1px solid ${result.isValid 
                          ...
                          <ListItemText
                            primary={
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                                ...
                                <Chip 
                                  label={result.isValid ? 'Valid' : 'Issues Found'} 
                                  size="small"
                        ...
                        {index < validationResults.length - 1 && <Divider sx={{ opacity: 0.3 }} />}
                      </React.Fragment>
                    ))}
                  </List>
                </Box>
          ...
          >
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                ...
                >
                  <TrendingUp sx={{ color: theme.palette.primary.main, fontSize: 24 }} />
                </Box>
                <Box>
                  <Typography variant="h6" sx={{ fontWeight: 700, mb: 0.5 }}>
                ...
                {Object.entries(qualityScores).map(([category, score]) => (
                  <Grid item xs={12} sm={6} md={3} key={category}>
                    ...
                    >
                      ...
                      <Typography variant="body2" sx={{ fontWeight: 600, mb: 2, textTransform: 'capitalize' }}>
