...
const TeamPerformanceWidget = ({ period = 'weekly', expanded = false }) => {
  ...
  const teamMembers = [
    { id: 1, name: 'Mike Johnson', role: 'Senior Technician', avatar: '/avatars/mike.jpg', experience: 8 },
    { id: 2, name: 'Sarah Chen', role: 'Body Repair Specialist', avatar: '/avatars/sarah.jpg', experience: 5 },
    { id: 3, name: 'David Rodriguez', role: 'Paint Technician', avatar: '/avatars/david.jpg', experience: 6 },
    { id: 4, name: 'Emily Watson', role: 'Parts Specialist', avatar: '/avatars/emily.jpg', experience: 4 },
    { id: 5, name: 'James Thompson', role: 'Quality Inspector', avatar: '/avatars/james.jpg', experience: 7 },
    { id: 6, name: 'Lisa Garcia', role: 'Service Advisor', avatar: '/avatars/lisa.jpg', experience: 3 }
  ];
  ...
  useEffect(() => {
    ...
    const generateTeamData = () => {
      ...
      const totalJobs = data.reduce((sum, member) => sum + member.completedJobs, 0);
      const avgEfficiency = Math.round(data.reduce((sum, member) => sum + member.efficiency, 0) / data.length);
      const avgQuality = Math.round(data.reduce((sum, member) => sum + member.quality, 0) / data.length);
      const avgRating = (data.reduce((sum, member) => sum + parseFloat(member.customerRating), 0) / data.length).toFixed(1);
      const totalHours = data.reduce((sum, member) => sum + member.hoursWorked, 0);
      const totalOvertime = data.reduce((sum, member) => sum + member.overtimeHours, 0);
      ...
      setTeamMetrics({
        ...
        topPerformer: data.reduce((prev, current) => 
          (prev.efficiency + prev.quality) > (current.efficiency + current.quality) ? prev : current
        ),
              ...
              badgeContent={
                <Avatar 
                  sx={{ 
                    width: 16, 
                    height: 16, 
                    bgcolor: member.trend === 'up' ? 'success.main' : 'warning.main'
                  }}
                >
                  {member.trend === 'up' ? 
              ...
              <LinearProgress
                ...
                sx={{
                  ...
                  '& .MuiLinearProgress-bar': {
                    borderRadius: 2,
                    bgcolor: member.efficiency > 85 ? 'success.main' : 
                             member.efficiency > 70 ? 'info.main' : 'warning.main'
                  }
                }}
              />
            </Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Button
            ...
            onClick={() => setSelectedView(
              selectedView === 'overview' ? 'individual' : 
              selectedView === 'individual' ? 'metrics' : 'overview'
            )}
            sx={{ minWidth: 120 }}
          >
            {selectedView === 'overview' ? 'Overview' : 
             selectedView === 'individual' ? 'Individual' : 'Metrics'}
          </Button>
      ...
      {selectedView === 'overview' && (
        <Box sx={{ mb: 3 }}>
          <Grid container spacing={2}>
            <Grid item xs={6} sm={3}>
              <Card sx={{ p: 2, ...getGlassStyles('subtle', mode), textAlign: 'center' }}>
        ...
        <AnimatePresence mode="wait">
          ...
          {selectedView === 'individual' && (
            ...
            >
              ...
              <Grid container spacing={2}>
                {teamData.map((member) => (
                  <Grid item xs={12} sm={6} md={expanded ? 4 : 6} lg={expanded ? 3 : 4} key={member.id}>
              ...
              <Grid container spacing={3}>
                  ...
                  <Card sx={{ p: 3, ...getGlassStyles('default', mode), height: '100%' }}>
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart data={teamData}>
                        ...
                        <Bar dataKey="quality" fill={theme.palette.success.main} name="Quality" />
                      </BarChart>
                    </ResponsiveContainer>
                  </Card>
                </Grid>
                ...
                <Grid item xs={12} md={4}>
                  <Card sx={{ p: 3, ...getGlassStyles('default', mode), height: '100%' }}>
                    ...
                    <List dense>
                      {teamData
                        .sort((a, b) => (b.efficiency + b.quality) - (a.efficiency + a.quality))
                        .slice(0, 3)
                        .map((member, index) => (
                          <ListItem key={member.id} sx={{ px: 0 }}>
                            ...
                            <ListItemText
                              ...
                              secondary={
                                <Typography variant="caption" color="text.secondary">
                                  {Math.round((member.efficiency + member.quality) / 2)}% avg
                                </Typography>
                              }
                            />
                          </ListItem>
