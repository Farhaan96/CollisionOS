...
const AdvancedKPIChart = React.memo(({ kpiData = {}, timeframe = 'month', showTrends = true }) => {
  const theme = useTheme();
  const [selectedKPIs, setSelectedKPIs] = useState(['revenue', 'cycleTime', 'customerSatisfaction', 'laborEfficiency']);
  ...
  const chartData = useMemo(() => {
    const datasets = selectedKPIs.map((kpiKey, index) => {
      ...
      return {
        ...
        backgroundColor: alpha(config?.color || theme.palette.primary.main, 0.1),
        fill: chartType === 'area',
  ...
  const KPICard = React.memo(({ kpiKey, data, isSelected, onClick }) => {
    const config = KPI_CONFIGS[kpiKey];
    const currentValue = data?.current || 0;
    const previousValue = data?.previous || 0;
    const change = useMemo(() => 
      previousValue ? ((currentValue - previousValue) / previousValue * 100) : 0, 
            ...
            {showTrends && (
              <Box sx={{ display: 'flex', alignItems: 'center', mt: 2 }}>
                {change > 0 ? (
                  ...
                  <TrendingFlat sx={{ color: 'grey.500', fontSize: 16, mr: 0.5 }} />
                )}
                <Typography
                  variant="body2"
                  color={change > 0 ? 'success.main' : change < 0 ? 'error.main' : 'text.secondary'}
                >
                  {Math.abs(change).toFixed(1)}% vs {timeframe === 'month' ? 'last month' : 'previous period'}
                </Typography>
              </Box>
            )}
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {performanceLevel === 'excellent' ? '100%' : 
                     ...
                     performanceLevel === 'fair' ? '50%' : '25%'}
                  </Typography>
                </Box>
                <LinearProgress
                  variant="determinate"
                  value={performanceLevel === 'excellent' ? 100 : 
                         performanceLevel === 'good' ? 75 :
                         performanceLevel === 'fair' ? 50 : 25}
                  sx={{
                    height: 6,
  ...
  }, [getPerformanceLevel, getPerformanceColor, formatKPIValue, theme.palette, isSelected, onClick]);
