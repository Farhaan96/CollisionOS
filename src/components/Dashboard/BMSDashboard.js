...
const BMSDashboard = () => {
  ...
  const mockBMSData = [
    {
      ...
      damage: {
        ...
        totalAmount: 2847.50,
        damageLines: [
          { part: 'Front Bumper', operation: 'Replace', labor: 2.5, parts: 450.00 },
          { part: 'Hood', operation: 'Repair', labor: 3.0, parts: 0.00 },
          { part: 'Left Fender', operation: 'Replace', labor: 2.0, parts: 320.00 }
        ]
      }
    },
    {
      ...
      damage: {
        ...
        totalAmount: 1956.75,
        damageLines: [
          { part: 'Rear Bumper', operation: 'Replace', labor: 2.0, parts: 380.00 },
          { part: 'Trunk Lid', operation: 'Repair', labor: 2.5, parts: 0.00 },
          { part: 'Right Quarter Panel', operation: 'Repair', labor: 4.0, parts: 0.00 }
        ]
      }
    }
  ];
  ...
  const handleUploadComplete = (data, file) => {
    ...
    const newBMSData = {
      ...
      documentInfo: {
        documentNumber: safeText(data.documentInfo?.documentNumber) || 'Unknown',
        documentType: safeText(data.documentInfo?.documentType) || 'Estimate',
        createdDate: new Date().toISOString().split('T')[0],
        status: 'Pending'
      },
      claimInfo: {
        claimNumber: safeText(data.claimInfo?.claimNumber) || 'CLM-' + Date.now(),
        insuranceCompany: safeText(data.claimInfo?.insuranceCompany) || 'Unknown',
      ...
      damage: {
        ...
        damageLines: Array.isArray(data.damage?.damageLines) ? data.damage.damageLines.map(line => ({
        ...
        <CardContent sx={{ p: 4 }}>
          ...
          <Grid container spacing={3}>
            {[
              ...
              { 
                value: bmsData.filter(bms => bms.documentInfo.status === 'Approved').length, 
                label: 'Approved', 
              ...
              { 
                value: bmsData.reduce((sum, bms) => sum + bms.damage.totalParts, 0), 
                label: 'Total Parts', 
              ...
              { 
                value: formatCurrency(bmsData.reduce((sum, bms) => sum + bms.damage.totalAmount, 0)), 
                label: 'Total Value', 
                ...
                >
                  ...
                  >
                    ...
                    <Typography variant="h3" sx={{ 
                      ...
                      {stat.isValue ? stat.value : <AnimatedCounter value={stat.value} />}
                    </Typography>
                    ...
                    <LinearProgress
                      variant="determinate"
                      value={Math.min((stat.isValue ? 100 : (stat.value || 0) * 20), 100)}
                      sx={{
                        mt: 2,
                        height: 4,
                        borderRadius: 2,
        ...
        <CardHeader
          ...
          subheader={
            <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}>
              {bms.documentInfo.documentType} • Created: {bms.documentInfo.createdDate}
            </Typography>
          }
          action={
            <IconButton
            ...
            >
              {expandedSections[`bms-${bms.id}`] ? <ExpandLess /> : <ExpandMore />}
            </IconButton>
          }
        />
        ...
        <Collapse in={expandedSections[`bms-${bms.id}`]}>
          <CardContent>
            <Grid container spacing={3}>
                  ...
                  <List dense>
                    <ListItem>
                      <ListItemText
                        ...
                        sx={{ '& .MuiListItemText-primary': { color: 'white', fontWeight: 600 } }}
                      />
                    </ListItem>
                    <ListItem>
                      <ListItemIcon><Phone sx={{ color: 'rgba(255,255,255,0.7)' }} /></ListItemIcon>
                      <ListItemText primary={bms.customer.phone} sx={{ '& .MuiListItemText-primary': { color: 'rgba(255,255,255,0.9)' } }} />
                    </ListItem>
                    <ListItem>
                      <ListItemIcon><Email sx={{ color: 'rgba(255,255,255,0.7)' }} /></ListItemIcon>
                      <ListItemText primary={bms.customer.email} sx={{ '& .MuiListItemText-primary': { color: 'rgba(255,255,255,0.9)' } }} />
                    </ListItem>
                    <ListItem>
                      <ListItemIcon><LocationOn sx={{ color: 'rgba(255,255,255,0.7)' }} /></ListItemIcon>
                      <ListItemText primary={bms.customer.address} sx={{ '& .MuiListItemText-primary': { color: 'rgba(255,255,255,0.9)' } }} />
                    </ListItem>
                  </List>
                </Paper>
              </Grid>
                  ...
                  <List dense>
                    ...
                    <ListItem>
                      <ListItemText
                        ...
                        secondary="VIN"
                        sx={{ '& .MuiListItemText-primary': { color: 'rgba(255,255,255,0.9)', fontFamily: 'monospace' } }}
                      />
                    </ListItem>
                    <ListItem>
                      <ListItemText
              ...
              {/* Damage Lines Table */}
              <Grid item xs={12}>
                <Paper sx={{ bgcolor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                  ...
                  <TableContainer>
                    <Table>
                      <TableHead>
                        <TableRow>
                          <TableCell sx={{ color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>Part</TableCell>
                          <TableCell sx={{ color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>Operation</TableCell>
                          <TableCell sx={{ color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>Labor (hrs)</TableCell>
                          <TableCell sx={{ color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>Parts Cost</TableCell>
                      ...
                      <TableBody>
                        {bms.damage.damageLines.map((line, idx) => (
                          <TableRow key={idx}>
                            <TableCell sx={{ color: 'white' }}>{line.part}</TableCell>
                            <TableCell>
                              <Chip
                                label={line.operation}
                                size="small"
                                color={line.operation === 'Replace' ? 'primary' : 'secondary'}
                                sx={{ fontWeight: 600 }}
                              />
                            </TableCell>
                            <TableCell sx={{ color: 'rgba(255,255,255,0.9)' }}>{line.labor}</TableCell>
                            <TableCell sx={{ color: 'rgba(255,255,255,0.9)' }}>
                              {formatCurrency(line.parts)}
                            </TableCell>
                            <TableCell sx={{ color: '#10B981', fontWeight: 600 }}>
                              {formatCurrency(line.labor * 50 + line.parts)} {/* Assuming $50/hr labor rate */}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        <Typography variant="h3" gutterBottom sx={{ fontWeight: 800, color: 'white' }}>
          BMS Files Dashboard
        </Typography>
        <Typography variant="h6" sx={{ color: 'rgba(255,255,255,0.7)' }}>
          Comprehensive view of all BMS (Body Management System) files and their details
        </Typography>
      </Box>
