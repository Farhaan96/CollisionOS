...
const ShortcutHelper = ({ open, onClose }) => {
  ...
  useEffect(() => {
    if (showKeyboard && currentTab === 1) {
      const keys = [];
      Object.values(filteredShortcuts)
        .flat()
        .forEach(shortcut => {
          const shortcutKeys = shortcut.display
            .split(/[+\s]/)
            .map(k => k.trim());
          ...
          <TabPanel value={currentTab} index={1}>
            <Box sx={{ px: 3 }}>
              <Typography variant='body1' sx={{ mb: 3, textAlign: 'center' }}>
                Visual representation of keyboard shortcuts. Highlighted keys
                are mapped to shortcuts.
              </Typography>

              <VisualKeyboard
                highlightedKeys={highlightedKeys}
                shortcuts={Object.values(filteredShortcuts)
                  .flat()
                  .reduce((acc, shortcut) => {
              ...
              <List>
                {Object.values(customShortcuts).map(shortcut => (
                  <ShortcutItem
                    ...
                    customizable
                    onEdit={shortcut =>
                      setCustomizeDialog({ open: true, shortcut })
                    }
                    onDelete={shortcut => {
