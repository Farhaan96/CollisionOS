...
const SmartAutocomplete = ({
  ...
  const saveRecentSelection = useCallback((selection) => {
    ...
    const newRecent = [
      ...
      ...recentSelections.filter(item => 
        getOptionValue(item) !== getOptionValue(selection)
      )
    ...
    try {
      localStorage.setItem(`autocomplete-recent-${label}`, JSON.stringify(newRecent));
    } catch (error) {
      console.error('Failed to save recent selection:', error);
    }
  }, [enableRecentSelections, recentSelections, maxRecentSelections, label, getOptionValue]);
  ...
  const toggleFavorite = useCallback((option) => {
    ...
    const isFavorite = favorites.some(fav => getOptionValue(fav) === optionValue);
    ...
    try {
      localStorage.setItem(`autocomplete-favorites-${label}`, JSON.stringify(newFavorites));
    } catch (error) {
  ...
  const loadAsyncData = useCallback(async (searchTerm) => {
      ...
      debounceTimeoutRef.current = setTimeout(() => {
        ...
        if (inputValue.trim() && inputValue.length >= minSearchLength) {
          setSearchHistory(prev => [
            inputValue,
            ...prev.filter(term => term !== inputValue)
          ].slice(0, 10));
        }
      }, debounceMs);
    }
  ...
  const filteredOptions = useMemo(() => {
    ...
    if (!inputValue && enableRecentSelections && recentSelections.length > 0) {
      const recentWithLabel = recentSelections.map(item => ({ ...item, _isRecent: true }));
      combinedOptions = [...recentWithLabel, ...combinedOptions.filter(option => 
        !recentSelections.some(recent => 
          getOptionValue(recent) === getOptionValue(option)
        )
      )];
    }
    ...
    if (enableFavorites && favorites.length > 0) {
      const favoritesWithLabel = favorites.map(item => ({ ...item, _isFavorite: true }));
      combinedOptions = [
        ...favoritesWithLabel,
        ...combinedOptions.filter(option => 
          !favorites.some(fav => 
            getOptionValue(fav) === getOptionValue(option)
          )
        )
      ];
    }
  ...
  const defaultRenderOption = useCallback((props, option, { selected }) => {
    const label = getOptionLabel(option);
    const matches = option._match?.matches || [];
    const isFavorite = enableFavorites && favorites.some(fav => 
      getOptionValue(fav) === getOptionValue(option)
    );
  ...
  const defaultRenderGroup = useCallback((params) => {
  ...
  return (
    <Box sx={{ position: 'relative', ...sx }}>
          ...
          >
            <Fade in timeout={200}>
              <Paper>
                ...
                {/* Create new option */}
                {enableCreateNew && inputValue && !filteredOptions.some(
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  {enableRecentSelections && recentSelections.length > 0 && (
                    <Tooltip title="Recent selections">
                      ...
                      >
                        <Badge badgeContent={recentSelections.length} max={9} color="primary">
                          <History />
                        </Badge>
                      </IconButton>
                    </Tooltip>
      ...
      <Collapse in={showHistory}>
        <Card sx={{ mt: 1, position: 'absolute', top: '100%', width: '100%', zIndex: 1300 }}>
          <CardContent>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
            <TextField
              ...
              onChange={(e) => setNewOptionData(prev => ({ ...prev, label: e.target.value }))}
              fullWidth
            />
