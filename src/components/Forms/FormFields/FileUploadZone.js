...
const uploadFileInChunks = async (file, onProgress, uploadUrl, chunkSize = 1024 * 1024) => {
...
const FileUploadZone = ({
  ...
  const validateFile = useCallback((file) => {
    ...
    // Check file size
    const maxSize = acceptedFileTypes?.[fileType.type]?.maxSize || maxFileSize;
  ...
  }, [files, multiple, maxFiles, disabled, enableFileValidation, validateFile, onChange, autoUpload, uploadUrl]);
  ...
  const uploadFiles = useCallback(async (filesToUpload = files) => {
    ...
    for (const fileItem of filesToUpload) {
      ...
      try {
        ...
        if (enableChunkedUpload && fileItem.size > chunkSize) {
          result = await uploadFileInChunks(fileItem.file, onProgress, uploadUrl, chunkSize);
        } else {
          // Simple upload
          const formData = new FormData();
        ...
        setFiles(prev => prev.map(f => 
          f.id === fileItem.id 
            ? { ...f, uploadStatus: 'success', uploadProgress: 100, uploadResult: result }
            : f
        ));
    ...
    setUploading(false);
  }, [files, uploadUrl, uploading, enableChunkedUpload, chunkSize, onUploadComplete, onUploadError]);
      ...
      >
        ...
        >
          <CloudUpload
            sx={{
              fontSize: 48,
              color: error ? 'error.main' : isDragOver ? 'primary.main' : 'text.secondary',
              mb: 2
            }}
          />
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: 1 }}>
            {acceptedFileTypes && (
              <Chip size="small" label={`Types: ${acceptedFileTypes.join(', ')}`} />
            )}
            <Chip size="small" label={`Max size: ${formatFileSize(maxFileSize)}`} />
            ...
            <Box sx={{ textAlign: 'center' }}>
              ...
              {previewFile.fileType.type !== 'image' && (
                <Box sx={{ py: 8, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <Avatar sx={{ width: 80, height: 80, mb: 2, bgcolor: `${previewFile.fileType.color}.main` }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {formatFileSize(previewFile.size)} • {previewFile.fileType.type}
                  </Typography>
                </Box>
              )}
              ...
              <Box sx={{ mt: 2, textAlign: 'left' }}>
                <Typography variant="subtitle2" sx={{ mb: 1 }}>File Details:</Typography>
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    <Typography variant="body2" color="text.secondary">Size:</Typography>
                    <Typography variant="body2">{formatFileSize(previewFile.size)}</Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="body2" color="text.secondary">Type:</Typography>
                    <Typography variant="body2">{previewFile.type || 'Unknown'}</Typography>
                  </Grid>
