...
const SmartForm = ({
  ...
  const progress = useMemo(() => {
    if (!enableMultiStep) {
      const validFields = Object.keys(schema.fields || {}).filter(
        key => !errors[key] && formData[key] !== undefined && formData[key] !== ''
      );
  ...
  const validateField = useCallback((fieldName, value, allValues = formData) => {
    ...
    return validationEngine.validateField(fieldName, value, fieldSchema, allValues);
  ...
  const validateForm = useCallback(async () => {
    ...
    if (validationEngine.validateCrossFields) {
      const crossFieldErrors = await validationEngine.validateCrossFields(formData, schema);
  ...
  const updateDependentFields = useCallback((changedField, newValue) => {
    ...
    dependencies.forEach(dep => {
      ...
      if (shouldShow) {
        ...
        updates[dep.field] = dep.clearOnHide !== false ? '' : formData[dep.field];
      }
    });
  ...
  // History management
  const addToHistory = useCallback((newFormData) => {
  ...
  }, [enableAutoSave, enableLocalStorage, storageKey, formData, currentStep, completedSteps, schema.onAutoSave]);
  ...
  const handleFieldChange = useCallback((fieldName, value, shouldValidate = true) => {
  ...
  }, [formData, validateForm, onSubmit, errors, completedSteps, currentStep, enableLocalStorage, storageKey]);
  ...
  // Render field helper
  const renderField = useCallback((fieldName, fieldSchema, stepIndex = null) => {
  ...
  }, [schema, conditionalFields, formData, handleFieldChange, touched, errors, isSubmitting, customFields]);
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              {enableAutoSave && (
                <Tooltip title={
                  ...
                  <Chip
                    size="small"
                    icon={isAutoSaving ? <Save /> : hasUnsavedChanges ? <Warning /> : <Check />}
                    label={
                      isAutoSaving 
                        ? 'Saving...' 
                        : hasUnsavedChanges 
                  ...
                  <StepContent>
                    <Box sx={{ mt: 2 }}>
                        ...
                        {currentStep === index && (
                          ...
                          >
                            {step.fields.map(fieldName => 
                              renderField(fieldName, schema.fields[fieldName], index)
                            )}
                          </motion.div>
                        )}
                      </AnimatePresence>
                      ...
                      <Box sx={{ mt: 3, display: 'flex', gap: 2 }}>
                        <Button
                          variant="contained"
                          onClick={index === steps.length - 1 ? handleSubmit : nextStep}
                          disabled={isSubmitting}
                          startIcon={index === steps.length - 1 ? <Check /> : <NavigateNext />}
                        >
