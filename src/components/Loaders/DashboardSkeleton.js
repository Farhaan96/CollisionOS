...
const KPICardSkeleton = ({ variant = 'default' }) => {
  ...
  const cardHeight =
    variant === 'compact' ? 120 : variant === 'detailed' ? 180 : 140;
    ...
    <motion.div variants={widgetVariants}>
      ...
      >
        ...
        <Box display='flex' alignItems='center' gap={2} mb={4} px={1}>
          <SkeletonLoader variant='avatar' size={32} animation='pulse' />
          {!isCollapsed && (
            <SkeletonLoader
              variant='text'
              width='120px'
      ...
      >
        <Box display='flex' gap={spacing} minHeight='100vh'>
          ...
          <Box flex={1} overflow='auto'>
            <Container
              maxWidth={effectiveLayout === 'executive' ? false : 'xl'}
            >
              {/* Header */}
              {showHeader && (
                <HeaderSkeleton showUser={!isMobile} showNotifications={true} />
              )}
