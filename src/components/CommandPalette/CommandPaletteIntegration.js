...
import { useKeyboardShortcut, useNavigationShortcuts } from '../../hooks/useKeyboardShortcut';
...
const CommandPaletteIntegration = () => {
  ...
  useEffect(() => {
    ...
    // Quick save shortcut
    registerShortcut('cmd+s', (event) => {
      window.dispatchEvent(new CustomEvent('quickSave', { detail: { event } }));
    }, {
      id: 'quick-save',
      name: 'Quick Save',
    ...
    registerShortcut('/', (event) => {
      ...
      if (document.activeElement?.tagName !== 'INPUT' && 
        ...
        window.dispatchEvent(new CustomEvent('focusSearch', { detail: { event } }));
      }
    }, {
      id: 'focus-search',
      name: 'Focus Search',
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('openShortcutHelper', handleOpenShortcutHelper);
    };
  }, []);
