...
const CommandPalette = ({ open, onClose }) => {
  ...
  const { commands, recentCommands, executeCommand, addToRecent } = useCommandProvider();
  ...
  const fuzzySearch = useCallback((items, query) => {
    ...
    const scored = items.map(item => {
      ...
      for (let i = 0; i < allText.length && queryIndex < queryChars.length; i++) {
  ...
  const filteredCommands = useMemo(() => {
    ...
    const others = availableCommands.filter(cmd => !recentCommands.includes(cmd.id));
  ...
  }, [open, filteredCommands, selectedIndex, searchQuery, currentCategory, onClose]);
  ...
  // Execute command
  const handleCommandExecute = useCallback(async (command) => {
    try {
      addToRecent(command.id);
      setCommandHistory(prev => [...prev, { ...command, executedAt: new Date() }]);
