  ...
  const filteredJobs = useMemo(() => {
    ...
    if (filters.technician !== 'all') {
      filtered = filtered.filter(
        job => job.technicianId === filters.technician
      );
    }
  ...
  const handleJobAction = (action, job) => {
    switch (action) {
      ...
      case 'resume':
        setJobs(prevJobs =>
          prevJobs.map(j =>
            j.id === job.id ? { ...j, onHold: false, holdReason: null } : j
          )
        );
        break;
      case 'assign':
      ...
      <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
        ...
        >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            {job.bottleneck && <Warning fontSize='small' color='error' />}
            <IconButton
              size='small'
              onClick={e => {
                e.stopPropagation();
        ...
        {job.technician ? (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            <Typography variant='caption'>{job.technician}</Typography>
          </Box>
        ) : (
          <Typography
            variant='caption'
    ...
    return (
      <Paper
        ...
        sx={{
          ...
          backgroundColor: stats.isBottleneck
            ? `${theme.palette.error.main}08`
            : 'background.paper',
          border: stats.isBottleneck ? 2 : 1,
          borderColor: stats.isBottleneck
            ? theme.palette.error.main
            : 'divider',
        }}
      >
    ...
    <Box className={className} {...props}>
        ...
        <Grid item xs={6} sm={3}>
          <Card>
            <CardContent sx={{ textAlign: 'center' }}>
              ...
              >
                {
                  Object.values(stageStats).filter(stat => stat.isBottleneck)
                    .length
                }
              </Typography>
              <Typography variant='body2' color='text.secondary'>
      ...
      >
        ...
        <MenuItem
          onClick={() =>
            handleJobAction(
              selectedJob?.onHold ? 'resume' : 'hold',
              selectedJob
            )
          }
        >
