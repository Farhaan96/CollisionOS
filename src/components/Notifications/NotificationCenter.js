...
const NotificationCenter = ({
  ...
  const allNotifications = useMemo(() => {
    ...
    const historyItems = showHistory
      ? history.map(n => ({ ...n, isHistory: true }))
      : [];
    return [...active, ...historyItems].sort(
  ...
  const getFilterBadgeCount = useCallback(
    filter => {
      switch (filter) {
        case FILTER_TYPES.UNREAD:
          return allNotifications.filter(n => !n.read).length;
        case FILTER_TYPES.TODAY:
          return allNotifications.filter(n => isToday(new Date(n.timestamp)))
            .length;
        default:
          return null;
      }
        ...
        >
          ...
          >
            ...
            <Box
              sx={{
                p: 2,
                borderBottom: `1px solid ${premiumColors.glass.white[15]}`,
              }}
            >
              <Stack
                direction='row'
              ...
              >
                <Stack direction='row' alignItems='center' spacing={1}>
                  <NotificationsRounded
                    sx={{ color: premiumColors.primary[500] }}
                  />
                    ...
                    <IconButton
                      ...
                      sx={{
                        color: doNotDisturb.enabled
                          ? premiumColors.semantic.warning.main
                          : 'inherit',
                      }}
                    >
                      {doNotDisturb.enabled ? (
                  ...
                  {[
                    ...
                    return (
                      <Chip
                        ...
                        variant={
                          selectedFilter === filter.key ? 'filled' : 'outlined'
                        }
                        sx={{
                          backgroundColor:
                            selectedFilter === filter.key
                              ...
                              : 'transparent',
                          color:
                            selectedFilter === filter.key ? 'white' : 'inherit',
                                      ...
                                      >
                                        <ListItem
                                          sx={{
                                            ...
                                            backgroundColor:
                                              !notification.read &&
                                              !notification.isHistory
                                                ? alpha(
                                                    premiumColors.primary[50],
                                                    0.1
                                                  )
                                                : 'transparent',
                                            borderLeft:
                                              !notification.read &&
                                              !notification.isHistory
                                                ? `3px solid ${premiumColors.primary[500]}`
                                                : '3px solid transparent',
                                            opacity: notification.isHistory
                                              ? 0.7
                                              : 1,
                                            '&:hover': {
                                              backgroundColor: alpha(
            ...
            <Box
              sx={{
                p: 2,
                borderTop: `1px solid ${premiumColors.glass.white[15]}`,
              }}
            >
          ...
          >
            ...
            {showSettings && (
              <MenuItem
                onClick={() => {
                  setMoreMenuAnchor(null); /* Open settings */
                }}
              >
