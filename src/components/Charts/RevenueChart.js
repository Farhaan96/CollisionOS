...
export const RevenueChart = React.memo(
  ({
    ...
    const chartColors = useMemo(() => {
      return series.map(
        s => s.color || premiumDesignSystem.colors.primary[500]
      );
    }, [series]);
    ...
    const handleExportCSV = useCallback(() => {
      exportToCSV(
        filteredData,
        `revenue_chart_${new Date().toISOString().split('T')[0]}`
      );
    }, [filteredData]);
              ...
              <ButtonGroup size='small' variant='outlined'>
                ...
                <Tooltip title='Combined Chart'>
                  <Button
                    ...
                    variant={
                      chartType === 'combined' ? 'contained' : 'outlined'
                    }
                  >
                    <BarChart />
                  </Button>
              ...
              <defs>
                {series.map((serie, index) => (
                  ...
                  >
                    <stop
                      offset='5%'
                      stopColor={serie.color || chartColors[index]}
                      stopOpacity={0.3}
                    />
                    <stop
                      offset='95%'
