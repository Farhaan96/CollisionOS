...
import { TrendingUp, TrendingDown, Info, Visibility } from '@mui/icons-material';
...
export const ExecutiveKPICard = React.memo(({
  ...
  const animatedValue = useCountUp(animated && isVisible ? value : value, 1500);
  ...
  useEffect(() => {
    ...
    const element = document.getElementById(`kpi-card-${title?.replace(/\s+/g, '-')}`);
        ...
        <Box sx={{ 
          ...
          <Typography
            ...
            sx={{
              ...
              letterSpacing: premiumDesignSystem.typography.letterSpacing.wider,
              mb: 1
            }}
          >
            {title}
