      ...
      >
        {[...Array(5)].map((_, i) => (
          <FlowIndicator 
            key={i} 
            isActive={isActive} 
            color={bottleneck ? premiumDesignSystem.colors.semantic.error.main : premiumDesignSystem.colors.primary[400]}
...
const ProductionStage = ({ 
  ...
  const utilizationPercentage = Math.round((stage.currentLoad / stage.capacity) * 100);
      ...
      >
        <Card
          sx={{
            ...
            border: `2px solid ${isDragging ? premiumDesignSystem.colors.primary[300] : 
              isBottleneck ? premiumDesignSystem.colors.semantic.error.main :
        ...
        >
          ...
          <Box sx={{ mb: 1 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
              ...
              <Typography variant="caption" sx={{ 
                color: isBottleneck ? premiumDesignSystem.colors.semantic.error.main : premiumDesignSystem.colors.neutral[700],
                fontWeight: premiumDesignSystem.typography.fontWeight.semibold,
                fontSize: '0.7rem'
              }}>
            ...
            <LinearProgress
              ...
              sx={{
                ...
                '& .MuiLinearProgress-bar': {
                  backgroundColor: utilizationPercentage > 90 
...
export const ProductionFlowDiagram = React.memo(({
  ...
  const flowMetrics = useMemo(() => {
    const totalCapacity = stageOrder.reduce((sum, stage) => sum + stage.capacity, 0);
    const totalLoad = stageOrder.reduce((sum, stage) => sum + stage.currentLoad, 0);
    const averageUtilization = totalCapacity > 0 ? (totalLoad / totalCapacity) * 100 : 0;
    const activeStages = stageOrder.filter(stage => stage.status === 'active').length;
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              <Tooltip title="Average Utilization">
                <Chip
                  ...
                  size="small"
                  color={flowMetrics.averageUtilization > 85 ? "warning" : "success"}
                  variant="outlined"
                />
              </Tooltip>
              ...
              <Box
                ...
                sx={{
                  ...
                  backgroundColor: snapshot.isDraggingOver 
                    ...
                    : 'transparent',
                  borderRadius: premiumDesignSystem.borderRadius.lg,
                  transition: premiumDesignSystem.animations.transitions.colors
                }}
              >
                {stageOrder.map((stage, index) => (
                  <React.Fragment key={stage.id}>
                    ...
                    {index < stageOrder.length - 1 && (
                      <FlowConnection
                        isActive={stage.status === 'active' && stageOrder[index + 1].status === 'active'}
                        progress={connections[index]?.progress || Math.random() * 100}
                        bottleneck={stage.bottleneck || stageOrder[index + 1].bottleneck}
                      />
                    )}
                  </React.Fragment>
                ))}
        ...
        <AnimatePresence>
          {bottlenecks.length > 0 && (
            ...
            >
              ...
              >
                <Typography variant="subtitle2" sx={{ mb: 1, color: premiumDesignSystem.colors.semantic.warning.dark }}>
            ...
            <>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {statusConfig[selectedStage.status]?.label || 'Unknown Status'}
                    </Typography>
                  </Box>
                </Box>
              </DialogTitle>
              <DialogContent>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, mt: 2 }}>
                  <Box>
                    <Typography variant="subtitle2" gutterBottom>Capacity Utilization</Typography>
                    <LinearProgress
                      variant="determinate"
                      value={(selectedStage.currentLoad / selectedStage.capacity) * 100}
                      sx={{ height: 8, borderRadius: 4, mb: 1 }}
                    />
                    <Typography variant="body2" color="text.secondary">
                      {selectedStage.currentLoad} / {selectedStage.capacity} jobs
                    </Typography>
                  </Box>
                  <Box>
