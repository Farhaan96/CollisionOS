...
const BMSImport = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post('/api/import/bms', formData, {
        headers,
  ...
  const handleDrop = (event) => {
    ...
    if (file && (file.name.toLowerCase().endsWith('.xml') || file.name.toLowerCase().endsWith('.bms'))) {
      setSelectedFile(file);
      setError(null);
      setUploadResult(null);
    } else {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="body1" color="textSecondary" paragraph>
        Upload BMS (Body Management System) files to automatically create jobs and estimates.
        Supports XML and BMS file formats from major estimating systems.
      </Typography>
        ...
        <CardContent>
          ...
          {uploadResult && (
            <Alert severity="success" sx={{ mb: 2 }} icon={<CheckCircle />}>
              ...
              {uploadResult.jobsCreated && ` Created ${uploadResult.jobsCreated} job(s).`}
            </Alert>
          )}
