...
const partsInventory = [
  { part: 'Front Bumper - Honda Civic', quantity: 2, cost: 450.00, supplier: 'Honda Parts', status: 'In Stock' },
  { part: 'Hood - Toyota Camry', quantity: 1, cost: 320.00, supplier: 'Toyota Parts', status: 'Low Stock' },
  { part: 'Rear Bumper - Ford F-150', quantity: 0, cost: 580.00, supplier: 'Ford Parts', status: 'Out of Stock' },
  { part: 'Fender - BMW 3-Series', quantity: 3, cost: 420.00, supplier: 'BMW Parts', status: 'In Stock' },
  { part: 'Headlight Assembly - Chevy Silverado', quantity: 1, cost: 380.00, supplier: 'GM Parts', status: 'Low Stock' }
];

const technicianSchedule = [
  { name: 'Mike Johnson', currentJob: '2457 - Honda Civic', status: 'Body Work', efficiency: 156, hours: 8.5 },
  { name: 'Sarah Davis', currentJob: '2458 - Toyota Camry', status: 'Paint Prep', efficiency: 145, hours: 7.0 },
  { name: 'Tom Wilson', currentJob: '2460 - BMW 3-Series', status: 'Assembly', efficiency: 138, hours: 9.0 },
  { name: 'Lisa Brown', currentJob: '2461 - Chevy Silverado', status: 'QC', efficiency: 132, hours: 6.5 }
];

// Memoized KPI Card Component for better performance
const ProfessionalKPICard = React.memo(({ title, value, prefix = '', suffix = '', trend, icon, color, subtext, isLoading = false }) => {
  const trendColor = title.includes('Cycle Time') || title.includes('Touch Time') 
  ...
  return (
    <GlassCard sx={{ 
      ...
      <CardContent sx={{ p: 3 }}>
        ...
        <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.8)', mb: 2, fontWeight: 500 }}>
          {title}
        </Typography>
    ...
    <Box sx={{ width: '100%', height: 300, minWidth: 0 }}>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={revenueData}>
          ...
          />
          <Area type="monotone" dataKey="labor" stackId="1" stroke="#3B82F6" fill="url(#laborGradient)" />
          <Area type="monotone" dataKey="parts" stackId="1" stroke="#10B981" fill="url(#partsGradient)" />
        </AreaChart>
    </ResponsiveContainer>
    </Box>
  </GlassCard>
...
const AlertsPanel = React.memo(() => (
  <GlassCard sx={{ height: '100%', p: 3, overflow: 'hidden', maxWidth: '100%' }}>
    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3} sx={{ width: '100%', overflow: 'hidden' }}>
      <Typography variant="h6" sx={{ fontWeight: 700, color: 'white', flex: 1, minWidth: 0 }}>
        Live Alerts
      </Typography>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.2)' }}>
            <th style={{ textAlign: 'left', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Job #</th>
            <th style={{ textAlign: 'left', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Customer</th>
            <th style={{ textAlign: 'left', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Vehicle</th>
            <th style={{ textAlign: 'left', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Status</th>
            <th style={{ textAlign: 'right', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Value</th>
            <th style={{ textAlign: 'center', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Days</th>
          </tr>
        </thead>
        <tbody>
          {recentJobs.map((job) => (
            <tr key={job.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
              <td style={{ padding: '8px', color: 'white', fontWeight: 600 }}>{job.id}</td>
              <td style={{ padding: '8px', color: 'rgba(255,255,255,0.9)' }}>{job.customer}</td>
              <td style={{ padding: '8px', color: 'rgba(255,255,255,0.9)' }}>{job.vehicle}</td>
              <td style={{ padding: '8px' }}>
                <Chip 
                  label={job.status} 
                  size="small"
                  sx={{ 
                    bgcolor: job.status === 'Estimate' ? '#3B82F6' : 
                            job.status === 'Body Work' ? '#8B5CF6' : 
                            job.status === 'Paint Prep' ? '#06B6D4' : 
                            job.status === 'Assembly' ? '#10B981' : '#F59E0B',
                    color: 'white',
                    fontWeight: 600
                  }}
                />
              ...
              <td style={{ padding: '8px', textAlign: 'center', color: 'rgba(255,255,255,0.9)' }}>{job.days}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  <GlassCard sx={{ height: '100%', p: 3 }}>
    ...
    <Box sx={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.2)' }}>
            <th style={{ textAlign: 'left', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Part</th>
            <th style={{ textAlign: 'center', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Qty</th>
            <th style={{ textAlign: 'right', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Cost</th>
            <th style={{ textAlign: 'left', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Supplier</th>
            <th style={{ textAlign: 'center', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Status</th>
          </tr>
        </thead>
        <tbody>
          {partsInventory.map((part, index) => (
            <tr key={index} style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
              <td style={{ padding: '8px', color: 'rgba(255,255,255,0.9)' }}>{part.part}</td>
              <td style={{ padding: '8px', textAlign: 'center', color: 'rgba(255,255,255,0.9)' }}>{part.quantity}</td>
              <td style={{ padding: '8px', textAlign: 'right', color: '#F59E0B', fontWeight: 600 }}>
                ${part.cost.toLocaleString()}
              </td>
              <td style={{ padding: '8px', color: 'rgba(255,255,255,0.9)' }}>{part.supplier}</td>
              <td style={{ padding: '8px', textAlign: 'center' }}>
                <Chip 
                  label={part.status} 
                  size="small"
                  sx={{ 
                    bgcolor: part.status === 'In Stock' ? '#10B981' : 
                            part.status === 'Low Stock' ? '#F59E0B' : '#EF4444',
                    color: 'white',
                    fontWeight: 600
                  }}
                />
  ...
  <GlassCard sx={{ height: '100%', p: 3 }}>
    ...
    <Box sx={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.2)' }}>
            <th style={{ textAlign: 'left', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Technician</th>
            <th style={{ textAlign: 'left', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Current Job</th>
            <th style={{ textAlign: 'left', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Status</th>
            <th style={{ textAlign: 'center', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Efficiency</th>
            <th style={{ textAlign: 'center', padding: '8px', color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>Hours</th>
          </tr>
        </thead>
        <tbody>
          {technicianSchedule.map((tech, index) => (
            <tr key={index} style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
              <td style={{ padding: '8px', color: 'white', fontWeight: 600 }}>{tech.name}</td>
              <td style={{ padding: '8px', color: 'rgba(255,255,255,0.9)' }}>{tech.currentJob}</td>
              <td style={{ padding: '8px' }}>
                <Chip 
                  label={tech.status} 
                  size="small"
                  sx={{ 
                    bgcolor: tech.status === 'Body Work' ? '#8B5CF6' : 
                            tech.status === 'Paint Prep' ? '#06B6D4' : 
                            tech.status === 'Assembly' ? '#10B981' : '#F59E0B',
                    color: 'white',
                    fontWeight: 600
                  }}
                />
              ...
              <td style={{ padding: '8px', textAlign: 'center', color: 'rgba(255,255,255,0.9)' }}>{tech.hours}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export const DashboardPage = () => {
  ...
  const [techPerformance, setTechPerformance] = useState(realTimeData.laborEfficiency.series);
  ...
  const loadDashboardData = async (timeframe = 'month') => {
    setLoading(true);
    try {
      const [kpisData, productionData, revenueData, jobsData, alertsData, techData] = await Promise.all([
      ...
      console.log('Dashboard data loaded:', { kpisData, productionData, revenueData, jobsData, alertsData, techData });
    } catch (error) {
    ...
    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={4}>
      <Box>
        ...
        <Typography variant="h6" sx={{ 
          ...
          Executive Dashboard • {currentTime.toLocaleDateString()} • {currentTime.toLocaleTimeString()}
        </Typography>
      </Box>
      <Button
        startIcon={<Refresh />}
      ...
      <Container maxWidth="xl" sx={{ py: 4, position: 'relative', zIndex: 1 }}>
        ...
        <BentoGrid cols={12} gap={3}>
          ...
          <BentoItem span={{ xs: 12, sm: 6, md: 4 }}>
            <GlassCard sx={{ height: '100%', cursor: 'pointer' }} onClick={() => navigate('/bms-dashboard')}>
