...
const BMSImportPage = () => {
  ...
  const handleUploadComplete = (data, file) => {
    ...
    setImportStats(prev => ({
      ...
      totalValue: prev.totalValue + (data.job?.totalAmount || data.damage?.totalAmount || 0)
    }));
  ...
  const calculateSectionQualityScore = (sectionValidation) => {
    ...
    const { errors = [], warnings = [], fieldValidations = {} } = sectionValidation;
  ...
  const calculateOverallQualityScore = (validationResults) => {
    ...
    const scores = sections.map(section => calculateSectionQualityScore(section));
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      >
        <Box sx={{ mb: 6, textAlign: 'center' }}>
          ...
          <Typography variant="h6" color="text.secondary" sx={{ maxWidth: 600, mx: 'auto', opacity: 0.9 }}>
            Import Mitchell Estimating BMS XML files to automatically create customers, vehicles, and jobs with intelligent data parsing
          </Typography>
      ...
      >
        <Grid container spacing={3} sx={{ mb: 6 }}>
          {[
            { key: 'database', icon: DataObject, label: 'Database', status: systemStatus.database },
            { key: 'storage', icon: Analytics, label: 'Storage', status: systemStatus.storage },
            { key: 'processing', icon: Speed, label: 'Processing', status: systemStatus.processing }
          ].map((item, index) => (
                  ...
                  >
                    <CardContent sx={{ p: 3 }}>
                      ...
                      <Typography variant="h3" sx={{ 
                        ...
                        {typeof stat.value === 'number' && stat.value === 0 ? '0' : stat.value}
                      </Typography>
                ...
                <Box sx={{ p: 3, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.1)}` }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      ...
                      >
                        <Timeline sx={{ color: theme.palette.info.main, fontSize: 24 }} />
                      </Box>
                      <Box>
                        <Typography variant="h6" sx={{ fontWeight: 700, mb: 0.5 }}>
                          Recent Imports
                ...
                <Box sx={{ p: 3 }}>
                  {recentImports.length === 0 ? (
                    <Box sx={{ textAlign: 'center', py: 6 }}>
                      ...
                      >
                        <Description sx={{ color: theme.palette.text.secondary, fontSize: 32 }} />
                      </Box>
                        ...
                        >
                          <Paper
                            sx={{
                              p: 2.5,
                              borderRadius: '12px',
                              background: importRecord.status === 'success'
                                ? alpha(theme.palette.success.main, 0.05)
                                : alpha(theme.palette.error.main, 0.05),
                              border: `1px solid ${importRecord.status === 'success' 
                              ...
                              '&:hover': {
                                ...
                                boxShadow: `0 4px 12px ${alpha(
                                  importRecord.status === 'success' ? theme.palette.success.main : theme.palette.error.main, 
                                  0.15
                                )}`
                              }
                            }}
                          >
                            <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                              ...
                              <Box sx={{ flex: 1, minWidth: 0 }}>
                                ...
                                {importRecord.status === 'success' && importRecord.customer && (
                                  <Typography variant="caption" sx={{ 
                                    color: theme.palette.success.main, 
                                    fontWeight: 500,
                                    display: 'block'
                                  }}>
                                    ✓ {importRecord.customer.firstName} {importRecord.customer.lastName}
                                  </Typography>
                                )}
        ...
        {processingStatus !== 'idle' && (
          ...
          >
            <BMSImportDashboard
              ...
              validationResults={validationResults ? Object.values(validationResults) : []}
      ...
      {/* Data Validation UI */}
      <AnimatePresence>
        {showValidationDetails && validationResults && Object.keys(validationResults).length > 0 && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
          ...
          <GlassCard>
            <CardContent>
              ...
              <Typography variant="body2" paragraph>
                BMS (Body Management System) files are XML documents created by Mitchell Estimating software 
                that contain detailed auto body repair estimates. These files include:
              </Typography>
      ...
      <Alert severity="info" sx={{ mt: 4 }}>
        <Typography variant="body2">
          <strong>Need help?</strong> BMS files must be in the standard Mitchell Estimating XML format. 
          If you're experiencing issues, please ensure your files are valid BMS XML documents and contact support if needed.
        </Typography>
      </Alert>
    </Container>
  );
