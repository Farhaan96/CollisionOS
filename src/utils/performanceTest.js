...
const mockJobs = [
  { id: '1', jobNumber: 'JOB-001', customerName: 'John Doe', status: 'estimate' },
  { id: '2', jobNumber: 'JOB-002', customerName: 'Jane Smith', status: 'in_progress' },
  { id: '3', jobNumber: 'JOB-003', customerName: 'Bob Wilson', status: 'ready_pickup' }
];

const mockAlerts = [
  { id: 1, type: 'warning', title: 'Parts Overdue', message: '2 parts overdue' },
...
export const runPerformanceTests = async () => {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      rerender(<RecentJobs jobs={[...mockJobs, { id: `${i}`, jobNumber: `JOB-${i}`, customerName: `Test ${i}`, status: 'estimate' }]} />);
    }
    ...
    const benchmarks = {
      renderTime: results.kpiChart < 50 && results.recentJobs < 30 && results.alertsPanel < 25,
...
export const performanceRegressionTest = (baseline, current) => {
  ...
  Object.entries(baseline).forEach(([metric, baselineValue]) => {
    ...
    if (currentValue && typeof baselineValue === 'number' && typeof currentValue === 'number') {
      const diff = ((currentValue - baselineValue) / baselineValue) * 100;
      
      if (diff > 20) { // More than 20% slower
        regressions.push({ metric, diff: diff.toFixed(1), baseline: baselineValue, current: currentValue });
      } else if (diff < -10) { // More than 10% faster
        improvements.push({ metric, diff: Math.abs(diff).toFixed(1), baseline: baselineValue, current: currentValue });
      }
    }
  });
  ...
  if (improvements.length > 0) {
    ...
    improvements.forEach(({ metric, diff, baseline, current }) => {
      console.log(`  📈 ${metric}: ${diff}% faster (${baseline}ms → ${current}ms)`);
    });
  }
  ...
  if (regressions.length > 0) {
    ...
    regressions.forEach(({ metric, diff, baseline, current }) => {
      console.log(`  📉 ${metric}: ${diff}% slower (${baseline}ms → ${current}ms)`);
    });
  }
