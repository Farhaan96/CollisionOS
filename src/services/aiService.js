...
class AIService {
  ...
  async query(query, context = {}) {
    try {
      ...
      if (error.response?.data) {
        ...
        return {
          type: 'error',
          message: error.response.data.message || 'I encountered an issue processing your request.',
          insights: ['Please try rephrasing your question or contact support if the issue persists.'],
  ...
  getDefaultSuggestions() {
    return [
      ...
      { type: 'analytics', query: 'How many jobs did we complete this month?', priority: 'medium' }
    ];
  }
    ...
    return [
      {
        name: 'Search',
        icon: '🔍',
        suggestions: this.getDefaultSuggestions().filter(s => s.type === 'search')
      },
      {
        name: 'Analytics',
        icon: '📊',
        suggestions: this.getDefaultSuggestions().filter(s => s.type === 'analytics')
      },
      {
        name: 'Workflow',
        icon: '🔄',
        suggestions: this.getDefaultSuggestions().filter(s => s.type === 'workflow')
      }
    ];
  }
  ...
  getKnowledge(term) {
    const knowledge = {
      'bms': {
        ...
        definition: 'Software used to import and process insurance estimates in XML format, streamlining the workflow from estimate to repair order.',
        examples: ['CCC ONE', 'Mitchell', 'Audatex'],
        relatedTerms: ['XML', 'Estimate', 'Integration']
      },
      'drp': {
        term: 'Direct Repair Program (DRP)',
        definition: 'A partnership between auto body shops and insurance companies for preferred repairs, often with negotiated rates and streamlined processes.',
        benefits: ['Guaranteed work', 'Faster payments', 'Reduced paperwork'],
        relatedTerms: ['Insurance', 'Partnership', 'Preferred Provider']
      },
      'supplement': {
        term: 'Supplement',
        definition: 'Additional work discovered during repair that wasn\'t included in the original estimate. Requires approval from the insurance company.',
        process: ['Discover additional damage', 'Document findings', 'Request approval', 'Proceed with work'],
        relatedTerms: ['Estimate', 'Additional Damage', 'Authorization']
      },
      'cycle time': {
        term: 'Cycle Time',
        definition: 'The total time a vehicle spends in the repair facility from check-in to completion.',
        importance: 'Key performance indicator for shop efficiency and customer satisfaction',
