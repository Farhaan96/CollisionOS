...
function generateCustomers(count = 15) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const customerType = randomChoice([
      'individual',
      'individual',
      'individual',
      'business',
    ...
    const customer = {
      ...
      companyName:
        customerType !== 'individual'
          ? `${lastName} ${randomChoice(['Corp', 'Ltd', 'Inc', 'Enterprises', 'Services'])}`
          : null,
  ...
  for (let i = 0; i < count; i++) {
    ...
    const job = {
      ...
      paymentMethod: isInsurance
        ? 'insurance'
        : randomChoice(['cash', 'credit_card', 'debit_card']),
...
function generateAllSampleData() {
  ...
  const categoryCount = {};
  parts.forEach(
    p => (categoryCount[p.category] = (categoryCount[p.category] || 0) + 1)
  );
