...
function generateCustomers() {
  ...
  for (let i = 0; i < CUSTOMERS_COUNT; i++) {
    const customerType = faker.helpers.arrayElement([
      'individual',
      'business',
      'fleet',
    ]);
    ...
    const customer = {
      ...
      paymentTerms:
        customerType === 'business' || customerType === 'fleet'
          ? faker.helpers.arrayElement(['net_15', 'net_30', 'net_60'])
          : 'immediate',
      ...
      firstVisitDate: faker.date.between({
        from: '2023-01-01',
        to: new Date(),
      }),
...
function generateVehicles(customers) {
  ...
  customers.forEach((customer, customerIndex) => {
    const vehicleCount = Math.floor(
      VEHICLES_PER_CUSTOMER + (Math.random() > 0.8 ? 1 : 0)
    );
...
function generateJobs(customers, vehicles) {
  ...
  for (let i = 0; i < JOBS_COUNT; i++) {
    ...
    const laborAmount = faker.number.float({
      min: 200,
      max: 5000,
      fractionDigits: 2,
    });
    const partsAmount = faker.number.float({
      min: 100,
      max: 8000,
      fractionDigits: 2,
    });
    const materialsAmount = faker.number.float({
      min: 50,
      max: 1000,
      fractionDigits: 2,
    });
    ...
    const deductible = isInsurance
      ? faker.number.float({ min: 250, max: 2000, fractionDigits: 2 })
      : 0;
    ...
    const job = {
      ...
      priority,
      jobType,
      claimNumber: isInsurance
        ? `CLM-24-${String(i + 1).padStart(4, '0')}`
        : null,
      deductible,
      customerPay,
      insurancePay,
      ...
      actualDeliveryDate:
        status === 'delivered'
          ? faker.date.between({ from: checkInDate, to: new Date() })
          : null,
      checkInDate,
      startDate: [
        'body_structure',
      ...
      paymentMethod: isInsurance
        ? 'insurance'
        : faker.helpers.arrayElement(['cash', 'credit_card', 'debit_card']),
      ...
      calibrationStatus:
        jobType === 'glass' || faker.datatype.boolean({ probability: 0.2 })
          ? 'pending'
          : 'not_required',
      photosRequired: true,
      ...
      documentsReceived:
        isInsurance && !['estimate', 'intake'].includes(status),
      authorizationReceived: !['estimate'].includes(status),
      authorizationDate: !['estimate'].includes(status)
        ? faker.date.between({ from: checkInDate, to: new Date() })
        : null,
...
function generateParts() {
  ...
  for (let i = 0; i < PARTS_COUNT; i++) {
    ...
    const part = {
      ...
      oemPartNumber: faker.datatype.boolean({ probability: 0.7 })
        ? generateOEMPartNumber(make)
        : null,
      ...
      partStatus: faker.helpers.arrayElement([
        'active',
        'discontinued',
        'special_order',
      ]),
      ...
      compatibility: {
        ...
        models: faker.helpers.arrayElements(vehicleModels[make] || [], {
          min: 1,
          max: 3,
        }),
      },
...
function generateOEMPartNumber(make) {
  const patterns = {
    Honda: () =>
      `${faker.string.numeric(5)}-${faker.string.alpha(3).toUpperCase()}-${faker.string.alphanumeric(3).toUpperCase()}`,
    Toyota: () => `${faker.string.numeric(5)}-${faker.string.numeric(5)}`,
    Ford: () =>
      `${faker.string.alpha(1).toUpperCase()}${faker.string.numeric(3)}-${faker.string.numeric(2)}-${faker.string.numeric(3)}`,
    BMW: () =>
      `${faker.string.numeric(2)} ${faker.string.numeric(2)} ${faker.string.numeric(1)} ${faker.string.numeric(6)}`,
    'Mercedes-Benz': () =>
      `A${faker.string.numeric(3)}${faker.string.numeric(3)}${faker.string.numeric(2)}${faker.string.numeric(2)}`,
  };
...
function generateAllSampleData() {
  console.log(
    'Generating comprehensive sample data for CollisionOS BMS system...\n'
  );
...
if (require.main === module) {
  ...
  console.log('\n=== Financial Summary ===');
  console.log(
    `Total Estimate Value: $${sampleData.stats.totalEstimateValue.toLocaleString('en-CA', { minimumFractionDigits: 2 })}`
  );
  console.log(
    `Average Job Value: $${(sampleData.stats.totalEstimateValue / sampleData.stats.jobs).toLocaleString('en-CA', { minimumFractionDigits: 2 })}`
  );
