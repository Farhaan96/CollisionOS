...
(function() {
  ...
  function initClickFix() {
    ...
    document.addEventListener('click', function(event) {
      ...
      if (actualTarget && (actualTarget.id === 'root' || actualTarget.tagName === 'DIV')) {
        // Look for buttons in a small radius around the click
        const buttons = document.querySelectorAll('button, [role="button"]');
        
        for (const button of buttons) {
          const rect = button.getBoundingClientRect();
          const inBounds = clickX >= rect.left && clickX <= rect.right && 
                          clickY >= rect.top && clickY <= rect.bottom;
          
          if (inBounds && button !== actualTarget) {
            console.log('🎯 Redirecting intercepted click to button:', button);
