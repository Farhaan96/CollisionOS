...
router.get('/', [
  query('status').optional().isIn(['draft', 'sent', 'acknowledged', 'shipped', 'received', 'cancelled']),
...
router.put('/:id/status', [
  param('id').isString().notEmpty(),
  body('status').isIn(['draft', 'sent', 'acknowledged', 'shipped', 'received', 'cancelled']),
...
router.post('/:id/receive', [
  ...
  body('items.*.condition').optional().isIn(['good', 'damaged', 'wrong_part'])
], handleValidationErrors, async (req, res) => {
  try {
    const { id } = req.params
