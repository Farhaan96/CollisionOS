...
async function testAuthentication(page) {
  ...
  try {
    ...
    if (isLoginPage) {
      ...
      await page.fill(
        'input[type="text"], input[type="email"], input[placeholder*="user" i], input[placeholder*="email" i]',
        'admin'
      );
...
async function testDashboard(page) {
  ...
  try {
    ...
    const dashboardVisible = await page
      .locator('text=/Dashboard|Overview|Statistics/i')
      .isVisible()
      .catch(() => false);
    ...
    if (dashboardVisible) {
      ...
      const kpiCards = await page
        .locator('[class*="card" i], [class*="stat" i], [class*="metric" i]')
        .count();
      if (kpiCards > 0) {
      ...
      const charts = await page
        .locator('canvas, svg[class*="chart" i], [class*="chart" i]')
        .count();
      if (charts > 0) {
...
async function testBMSImport(page) {
  ...
  try {
    ...
    const bmsPageVisible = await page
      .locator('text=/BMS|Import|Upload/i')
      .isVisible()
      .catch(() => false);
...
async function testCustomerManagement(page) {
  ...
  try {
    ...
    const customersVisible = await page
      .locator('text=/Customer|Client/i')
      .isVisible()
      .catch(() => false);
    ...
    if (customersVisible) {
      ...
      const customerData = await page
        .locator('table, [class*="list" i], [class*="grid" i]')
        .isVisible()
        .catch(() => false);
      if (customerData) {
      ...
      const addButton = await page
        .locator('button:has-text(/Add|New|Create/i)')
        .isVisible()
        .catch(() => false);
      if (addButton) {
...
async function testProductionBoard(page) {
  ...
  try {
    ...
    const productionVisible = await page
      .locator('text=/Production|Board|Workflow/i')
      .isVisible()
      .catch(() => false);
    ...
    if (productionVisible) {
      ...
      const columns = await page
        .locator('[class*="column" i], [class*="lane" i], [class*="stage" i]')
        .count();
      if (columns > 0) {
...
async function testPartsManagement(page) {
  ...
  try {
    ...
    const partsVisible = await page
      .locator('text=/Parts|Inventory/i')
      .isVisible()
      .catch(() => false);
    ...
    if (partsVisible) {
      ...
      const partsList = await page
        .locator('table, [class*="parts" i]')
        .isVisible()
        .catch(() => false);
      if (partsList) {
...
async function testReports(page) {
  ...
  try {
    ...
    const reportsVisible = await page
      .locator('text=/Report|Analytics/i')
      .isVisible()
      .catch(() => false);
    ...
    if (reportsVisible) {
      ...
      const reportOptions = await page
        .locator('button, a')
        .filter({ hasText: /Generate|Export|Download/i })
        .count();
      if (reportOptions > 0) {
        logTest(
          'Report options available',
          'PASS',
          `Found ${reportOptions} report options`
        );
      } else {
...
async function testThemeToggle(page) {
  ...
  try {
    ...
    const themeToggle = await page
      .locator(
        '[aria-label*="theme" i], button:has-text(/dark|light|theme/i), [class*="theme" i]'
      )
      .first();
    ...
    if (await themeToggle.isVisible().catch(() => false)) {
      ...
      if (initialBgColor !== newBgColor) {
        ...
        logTest(
          'Theme toggle works',
          'WARN',
          'Theme toggle clicked but no visible change'
        );
      }
    } else {
...
async function testResponsiveness(page) {
  ...
  try {
    ...
    const mobileMenuVisible = await page
      .locator(
        '[class*="menu" i], [class*="hamburger" i], [aria-label*="menu" i]'
      )
      .isVisible()
      .catch(() => false);
