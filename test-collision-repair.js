...
const { testSupabaseConnection, isSupabaseAvailable, getSupabaseClient } = require('./server/config/supabase');
...
async function testCollisionRepairTables() {
  ...
  console.log('\n📊 Summary:');
  console.log(`  ✅ Available: ${availableTables.length}/${collisionRepairTables.length}`);
  console.log(`  ❌ Missing: ${missingTables.length}/${collisionRepairTables.length}`);
  ...
  const coreTablesAvailable = coreTables.every(table => availableTables.includes(table));
  ...
  if (availableTables.length === collisionRepairTables.length) {
    ...
    console.log('⚠️  CollisionOS Collision Repair System: PARTIALLY OPERATIONAL');
    console.log(`   ${availableTables.length}/${collisionRepairTables.length} tables available. Some features may be limited.`);
  } else {
    console.log('❌ CollisionOS Collision Repair System: LIMITED FUNCTIONALITY');
...
async function testCollisionWorkflow(supabase) {
  try {
    ...
    if (roError) {
      ...
      console.log(`    ✅ Repair orders query successful (${roData?.length || 0} records)`);
    }
    ...
    if (claimsError) {
      ...
      console.log(`    ✅ Claims query successful (${claimsData?.length || 0} records)`);
    }
    ...
    if (poError) {
      ...
      console.log(`    ✅ Parts orders query successful (${poData?.length || 0} records)`);
    }
