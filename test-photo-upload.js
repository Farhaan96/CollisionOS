...
const SAMPLE_PNG_BASE64 =
  'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAHIAQqnowAAAABJRU5ErkJggg==';
...
async function testPhotoUploadSystem() {
  ...
  try {
    ...
    const categoriesResponse = await axios.get(
      `${API_BASE_URL}/api/attachments/categories`,
      {
    ...
    console.log(
      '   ✅ Categories:',
      categoriesResponse.data.data.map(c => c.value).join(', '),
      '\n'
    );
    ...
    const fileResponse = await axios.get(
      `${API_BASE_URL}/api/attachments/file/${uploadedFile.attachment.id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
    ...
    console.log(
      '   ✅ File retrieved successfully, size:',
    ...
    const thumbnailResponse = await axios.get(
      `${API_BASE_URL}/api/attachments/file/${uploadedFile.attachment.id}?thumbnail=true`,
      {
        headers: { Authorization: `Bearer ${token}` },
        responseType: 'arraybuffer',
      }
    ...
    if (thumbnailResponse.status !== 200) {
      console.log(
        '   ⚠️ Thumbnail retrieval failed (this is expected if thumbnail generation failed)'
      );
    } else {
      console.log(
        '   ✅ Thumbnail retrieved successfully, size:',
        thumbnailResponse.data.length,
        'bytes\n'
      );
    }
    ...
    const bulkUploadResponse = await axios.post(
      `${API_BASE_URL}/api/attachments/bulk-upload`,
      bulkFormData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!bulkUploadResponse.data.success) {
      throw new Error(
        'Bulk upload failed: ' + JSON.stringify(bulkUploadResponse.data)
      );
    }

    console.log(
      '   ✅ Bulk upload successful:',
      bulkUploadResponse.data.data.summary
    );
    console.log();
    ...
    const deleteResponse = await axios.delete(
      `${API_BASE_URL}/api/attachments/${uploadedFile.attachment.id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
    ...
    [
      ...
      ...Array.from({ length: 3 }, (_, i) =>
        path.join(__dirname, `test-image-${i + 1}.png`)
      ),
    ].forEach(file => {
      try {
        fs.unlinkSync(file);
