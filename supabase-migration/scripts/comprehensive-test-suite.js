...
class ComprehensiveTestSuite {
  ...
  async cleanupTestData() {
    ...
    try {
      ...
      if (this.testData.testJobId) {
        await this.adminClient.from('jobs').delete().eq('id', this.testData.testJobId);
      }
      
      if (this.testData.testVehicleId) {
        await this.adminClient.from('vehicles').delete().eq('id', this.testData.testVehicleId);
      }
      
      if (this.testData.testCustomerId) {
        await this.adminClient.from('customers').delete().eq('id', this.testData.testCustomerId);
      }
      
      if (this.testData.testUserId) {
        await this.adminClient.from('users').delete().eq('user_id', this.testData.testUserId);
        await this.adminClient.auth.admin.deleteUser(this.testData.testUserId);
      }
      
      if (this.testData.testShopId) {
        await this.adminClient.from('shops').delete().eq('id', this.testData.testShopId);
      }
  ...
  async testDataIntegrity() {
    ...
    await this.runTest('Foreign Key Constraints', async () => {
      ...
      if (!error) {
        throw new Error('Foreign key constraint not enforced - invalid customer_id was accepted');
      }
    ...
    await this.runTest('Unique Constraints', async () => {
      ...
      if (!error) {
        throw new Error('Unique constraint not enforced - duplicate job number was accepted');
      }
  ...
  async testPerformance() {
    ...
    await this.runTest('Simple Query Performance', async () => {
      const startTime = Date.now();
    ...
    await this.runTest('Complex Join Performance', async () => {
      ...
      if (duration > 500) {
        throw new Error(`Complex query too slow: ${duration}ms (target: <500ms)`);
      }
    ...
    await this.runTest('Bulk Operation Performance', async () => {
      ...
      if (duration > 2000) {
        throw new Error(`Bulk insert too slow: ${duration}ms (target: <2000ms for 50 records)`);
      }
  ...
  async testAuthentication() {
    ...
    await this.runTest('User Authentication', async () => {
      ...
      // Create user
      const { data: authUser, error: createError } = await this.adminClient.auth.admin.createUser({
        email: testEmail,
      ...
      return { userId: authUser.user.id, sessionExists: !!signInData.session };
    }, 'authentication');
    ...
    await this.runTest('Row Level Security', async () => {
      ...
      if (error) {
        ...
        return { message: 'RLS correctly blocking unauthorized access', blocked: true };
      } else if (data && data.length > 0) {
        throw new Error('RLS not properly configured - unauthorized access allowed');
      }
    ...
    await this.runTest('Permission System', async () => {
      ...
      if (user.role === 'owner' && !user.permissions['dashboard.view']) {
        throw new Error('Owner user missing expected dashboard.view permission');
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
          if (!eventReceived) {
            clearTimeout(timeout);
            this.anonClient.removeChannel(channel);
            resolve({ message: 'Subscription created but no events triggered' });
          }
        }, 10000);
      });
    }, 'realtime');
  ...
  async testBusinessLogic() {
    // Job workflow test
    await this.runTest('Job Workflow Management', async () => {
      // Test status progression
      const statusProgression = ['intake', 'blueprint', 'body_structure', 'paint_prep', 'delivered'];
    ...
    await this.runTest('Dashboard Stats Function', async () => {
      ...
      if (error) {
        if (error.message.includes('function') && error.message.includes('does not exist')) {
          ...
          return { message: 'Dashboard function not yet implemented', skipped: true };
        }
        throw error;
      }
  ...
  async testMigrationValidation() {
    ...
    await this.runTest('Schema Validation', async () => {
      const requiredTables = ['shops', 'users', 'customers', 'vehicles', 'jobs', 'parts', 'vendors'];
      const tableResults = {};
    ...
    await this.runTest('Data Migration Completeness', async () => {
      const dataDir = path.join(__dirname, '..', 'data-export', 'exported-data');
      
      if (!fs.existsSync(dataDir)) {
        return { message: 'No export data found to compare against', skipped: true };
      }
  ...
  async runAllTests() {
    ...
    try {
      ...
      this.log(`📊 Test suite completed in ${(duration / 1000).toFixed(2)} seconds`, 'success');
