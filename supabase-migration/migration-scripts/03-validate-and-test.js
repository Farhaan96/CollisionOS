...
class MigrationValidator {
  ...
  async testRLSPolicies() {
    ...
    for (const table of tables) {
      ...
      const { error } = await this.anonClient.from(table).select('*').limit(1);

      // We expect this to fail due to RLS
      if (!error) {
        this.log(
          `Warning: Table ${table} might not have proper RLS protection`,
          'warning'
        );
      }
    }
  ...
  async testForeignKeyConstraints() {
    ...
    try {
      ...
      if (!error) {
        throw new Error(
          'Foreign key constraint not working - invalid shop_id was accepted'
        );
      }
    } catch (insertError) {
  ...
  async testDataIntegrity() {
    ...
    for (const shop of shops || []) {
      ...
      if (error) {
        throw new Error(
          `Failed to query users for shop ${shop.id}: ${error.message}`
        );
      }
  ...
  async testUserRolePermissions() {
    ...
    for (const user of users || []) {
      if (!user.permissions || typeof user.permissions !== 'object') {
        throw new Error(
          `User ${user.user_id} has invalid permissions structure`
        );
      }
      ...
      if (user.role === 'owner' || user.role === 'admin') {
        if (!user.permissions['dashboard.view']) {
          throw new Error(
            `${user.role} user ${user.user_id} missing dashboard.view permission`
          );
        }
      }
    }
  ...
  async testBulkOperations() {
    ...
    if (insertDuration > 10000) {
      // 10 seconds
      throw new Error(`Bulk insert too slow: ${insertDuration}ms`);
    }

    this.log(
      `Bulk insert performance: ${insertDuration}ms for 100 records`,
      'info'
    );
  }
  ...
  async runAllTests() {
    ...
    await this.runTest('Foreign Key Constraints', () =>
      this.testForeignKeyConstraints()
    );
    await this.runTest('Database Functions', () =>
      this.testDatabaseFunctions()
    );

    // Data Integrity Tests
    await this.runTest('Data Integrity', () => this.testDataIntegrity());
    await this.runTest('User Role Permissions', () =>
      this.testUserRolePermissions()
    );
    await this.runTest('Job Status Workflow', () =>
      this.testJobStatusWorkflow()
    );

    // Authentication Tests
    await this.runTest('Authentication System', () =>
      this.testAuthenticationSystem()
    );
    await this.runTest('Password Reset', () => this.testPasswordReset());

    // Realtime Tests
    await this.runTest('Realtime Subscriptions', () =>
    ...
    // Business Logic Tests
    await this.runTest('Dashboard Stats Function', () =>
      this.testShopDashboardStats()
    );
    ...
    this.log(
      `Results: ${passed}/${total} tests passed (${summary.successRate}%)`,
