...
class EnhancedDataMigrator {
  ...
  async createBackup() {
    ...
    const backupFile = path.join(this.backupPath, `backup-${this.migrationId}.json`);
  ...
  async validatePreMigration() {
    ...
    try {
      const { data, error } = await this.supabase.from('_realtime').select('*').limit(1);
    ...
    if (!fs.existsSync(this.exportPath)) {
      throw new Error('Export data directory not found. Please run data export first.');
    }
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        for (let i = 0; i < data.length; i += batchSize) {
          ...
          this.log(`Processing ${tableName} batch ${batchNum}/${totalBatches} (${batch.length} records), retry ${retryCount}`);
          ...
          try {
            ...
            this.log(`Batch ${batchNum} failed: ${batchError.message}`, 'error');
        ...
        if (retryCount > maxRetries) {
          this.log(`Table ${tableName} migration failed after ${maxRetries} retries: ${tableError.message}`, 'error');
          failed += data.length;
          this.migrationState.recordsFailed += data.length;
          this.migrationState.tablesFailures.push(tableName);
          break;
        } else {
          this.log(`Retrying ${tableName} migration (attempt ${retryCount + 1}/${maxRetries + 1})`, 'warning');
  ...
  async migrateUser(user) {
    try {
      ...
      let authUser = null;
      try {
        const { data: existingUser } = await this.supabase.auth.admin.getUserById(user.id || user.user_id);
      ...
      if (!authUser) {
        const email = user.email || `${user.username}@placeholder.collisionos.com`;
      ...
      this.log(`User ${user.username} migration failed: ${error.message}`, 'error');
      throw error;
    }
  }

  generateSecurePassword() {
    return 'CollisionOS-Migrated-' + crypto.randomBytes(8).toString('hex') + '!';
  }
    ...
    for (const [tableName, tableData] of Object.entries(backupData)) {
      try {
        ...
        if (tableData.length > 0) {
          ...
          if (error) {
            this.log(`Failed to restore ${tableName}: ${error.message}`, 'error');
          } else {
            this.log(`Restored ${tableData.length} records to ${tableName}`, 'success');
          }
        }
      ...
      const channel = this.supabase
        ...
        .subscribe((status) => {
          if (status === 'SUBSCRIBED') {
            ...
            setTimeout(() => {
              this.supabase.from('jobs').select('id').limit(1).then(({ data }) => {
    ...
    try {
      ...
      for (const tableName of migrationOrder) {
        try {
          ...
          results[tableName] = await this.migrateTableWithRetry(tableName, batchSize);
        } catch (error) {
      ...
      const summaryFile = path.join(__dirname, '..', `enhanced-migration-summary-${this.migrationId}.json`);
      fs.writeFileSync(summaryFile, JSON.stringify(summary, null, 2));
      
      this.log(`Enhanced migration completed in ${duration.toFixed(2)} seconds`, 'success');
      ...
      if (includeBackup) {
        this.log('Backup is available. Use performRollback() method to restore previous state.', 'warning');
      }
  ...
  if (args.includes('--rollback')) {
    ...
    migrator.run(options)
      ...
      .catch(error => {
        ...
        console.error('📋 Use --rollback --migration-id=<id> to restore if needed');
        process.exit(1);
      });
  }
}
